...
export function summarize(results) {
  ...
  for (const rows of groups.values()) {
    ...
    summaries.push({
      ...
      peakRssBytes: median(rows.map((row) => row.serverResources?.peakRssBytes)),
      peakWorkingSetBytes: median(rows.map((row) => row.serverResources?.peakWorkingSetBytes)),
      peakPrivateMemoryBytes: median(rows.map((row) => row.serverResources?.peakPrivateMemoryBytes)),
      peakVirtualMemoryBytes: median(rows.map((row) => row.serverResources?.peakVirtualMemoryBytes)),
      avgCpuPercent: median(rows.map((row) => row.serverResources?.averageCpuPercent)),
      peakCpuPercent: median(rows.map((row) => row.serverResources?.peakCpuPercent)),
      peakThreads: median(rows.map((row) => row.serverResources?.peakThreads)),
      peakHandles: median(rows.map((row) => row.serverResources?.peakHandles)),
