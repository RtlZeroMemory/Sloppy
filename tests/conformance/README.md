...
| ENGINE-02 compiler metadata | `compiler/tests/fixtures/http-methods/input.js`, `async-handler/input.js`, `provider-capability/input.js`, `source-map/input.js` | Compile-artifact tests verify deterministic method metadata, async metadata, provider/capability plan metadata, and source-map artifacts. | `conformance.async_handler.run_once` executes the direct async handler fixture when V8 is enabled. Other metadata fixtures remain compile-only. | Non-GET dispatch, SQLite bridge enforcement, broader async handler source shapes, and source-map remapping remain later ENGINE layers. |
