...
foreach(public_template IN ITEMS api minimal-api program cli package-api node-compat)
    ...
    if(public_template STREQUAL "api")
        ...
        if(SLOPPY_ENABLE_V8)
            set(api_validation_problem_pattern "HTTP/1.1 400(.|\n|\r)*application/problem\\+json(.|\n|\r)*\"path\"(.|\n|\r)*\"code\"(.|\n|\r)*\"message\"")
            ...
            assert_sloppy_command_success("api template source POST /users missing field validation" "${public_project_dir}" "${api_validation_problem_pattern}" run .sloppy --header "content-type: application/json" --body-file missing-user-name.json --once POST /users)
            assert_sloppy_command_success("api template source POST /users invalid email validation" "${public_project_dir}" "${api_validation_problem_pattern}" run .sloppy --header "content-type: application/json" --body-file invalid-user-email.json --once POST /users)
            assert_sloppy_command_success("api template source POST /users malformed JSON validation" "${public_project_dir}" "${api_validation_problem_pattern}" run .sloppy --header "content-type: application/json" --body-file malformed-user.json --once POST /users)
        endif()
    endif()
    ...
    if(public_template STREQUAL "api")
        ...
        if(NOT EXISTS "${public_project_dir}/.sloppy/package/data")
            message(FATAL_ERROR "api template package did not create SQLite data directory")
        endif()
        ...
        if(SLOPPY_ENABLE_V8)
            assert_sloppy_command_success("api template packaged POST /users validation" "${public_project_dir}" "${api_validation_problem_pattern}" run .sloppy/package --header "content-type: application/json" --body-file missing-user-name.json --once POST /users)
        endif()
    endif()
    if(SLOPPY_ENABLE_V8)
        if(public_template STREQUAL "api")
            ...
            assert_sloppy_command_success("api template packaged /users absolute target from outside project" "${work_dir}" "HTTP/1.1 200" run "${public_project_dir}/.sloppy/package" --once GET /users)
        elseif(public_template STREQUAL "minimal-api")
