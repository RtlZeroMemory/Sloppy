...
function Get-IssueBody {
    ...
    $sourceDocs = if ($null -ne $Item.sourceDocs) { @($Item.sourceDocs) } else { @("AGENTS.md", "CONTRIBUTING.md", "docs/project/README.md", "docs/project/issue-workflow.md", "docs/project/pr-workflow.md", "docs/roadmap.md", "docs/quality-gates.md") }
    $goal = if (-not [string]::IsNullOrWhiteSpace([string]$Item.goal)) { @($Item.goal) } else { @($Item.summary) }
    $scope = if ($null -ne $Item.scope) { @($Item.scope) } elseif ($null -ne $Item.suggestedTasks) { @($Item.suggestedTasks) } else { @("Deliver the bounded issue slice described by the title and parent issue.", "Keep implementation aligned with the current source docs named in docs/project/README.md.") }
