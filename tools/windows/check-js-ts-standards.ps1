...
$stdlibRules = @(
    ...
    @{ Rule = "JS004"; Pattern = '(^|\s)import\s+.*["''](?:node:)?fs["'']|["'']node:fs["'']|(^|[^\w$.])require\s*\(\s*["'']fs["'']\s*\)'; Message = "Node fs usage is forbidden in bootstrap stdlib." },
    @{ Rule = "JS005"; Pattern = '(^|\s)import\s+.*["''](?:node:)?path["'']|["'']node:path["'']|(^|[^\w$.])require\s*\(\s*["'']path["'']\s*\)'; Message = "Node path usage is forbidden in bootstrap stdlib." },
