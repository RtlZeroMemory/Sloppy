...
const NODE_BUFFER_SHIM: &str = r#"const rt=globalThis.__sloppy_runtime||{};function enc(e="utf8"){const n=String(e).toLowerCase();if(n==="utf-8")return"utf8";if(["utf8","hex","base64"].includes(n))return n;throw new TypeError(`Sloppy Buffer encoding is not supported: ${e}`);}function fromString(value,e){switch(enc(e)){case"utf8":return rt.Text.utf8.encode(value);case"hex":return rt.Hex.decode(value);case"base64":return rt.Base64.decode(value);}}function toStringValue(value,e){switch(enc(e)){case"utf8":return rt.Text.utf8.decode(value);case"hex":return rt.Hex.encode(value);case"base64":return rt.Base64.encode(value);}}class Buffer extends Uint8Array{static from(value,encoding="utf8"){if(typeof value==="string")return new Buffer(fromString(value,encoding));if(value instanceof ArrayBuffer||ArrayBuffer.isView(value)||Array.isArray(value))return new Buffer(value);throw new TypeError("Buffer.from only supports string, ArrayBuffer, Uint8Array, or byte arrays in Sloppy.");}static alloc(size,fill=0){const buffer=new Buffer(size);buffer.fill(fill);return buffer;}static allocUnsafe(size){return Buffer.alloc(size);}static isBuffer(value){return value instanceof Buffer;}static byteLength(value,encoding="utf8"){return Buffer.from(value,encoding).byteLength;}static concat(list,totalLength=undefined){if(!Array.isArray(list))throw new TypeError("Sloppy Buffer.concat expects an array.");const buffers=list.map((entry)=>Buffer.from(entry));const length=totalLength===undefined?buffers.reduce((sum,entry)=>sum+entry.byteLength,0):Number(totalLength);if(!Number.isInteger(length)||length<0)throw new TypeError("Sloppy Buffer.concat totalLength must be a non-negative integer.");const output=new Buffer(length);let offset=0;for(const buffer of buffers){output.set(buffer.subarray(0,Math.max(0,length-offset)),offset);offset+=buffer.byteLength;if(offset>=length)break;}return output;}equals(other){const buffer=Buffer.from(other);if(buffer.byteLength!==this.byteLength)return false;for(let index=0;index<this.byteLength;index+=1){if(this[index]!==buffer[index])return false;}return true;}subarray(start=0,end=undefined){return new Buffer(super.subarray(start,end));}slice(start=0,end=undefined){return this.subarray(start,end);}toString(encoding="utf8"){return toStringValue(this,encoding);}}module.exports={Buffer,default:Buffer};"#;
...
const NODE_FS_SHIM: &str = r#"const promises=require("sloppy/node/fs/promises");function unsupported(name){return function(){throw new Error(`node:fs.${name} is not implemented by Sloppy's node:fs compatibility shim. Use sloppy/fs when available, or avoid this package path.`);};}module.exports={...promises,promises,existsSync:unsupported("existsSync"),watch:unsupported("watch"),default:null};module.exports.default=module.exports;"#;

const NODE_FS_PROMISES_SHIM: &str = r#"const rt=globalThis.__sloppy_runtime||{};function encoding(options){return typeof options==="string"?options:options?.encoding;}function sloppyPath(path){if(typeof path!=="string")return path;const value=path.replace(/\\/g,"/");if(value.startsWith("/")||value.startsWith("./")||value.startsWith("../")||value.includes("://"))return value;return `./${value}`;}async function readFile(path,options=undefined){const e=encoding(options);const target=sloppyPath(path);return e==="utf8"||e==="utf-8"?rt.File.readText(target):rt.File.readBytes(target);}async function writeFile(path,data,options=undefined){const e=encoding(options);const target=sloppyPath(path);if(typeof data==="string"||e==="utf8"||e==="utf-8")return rt.File.writeText(target,String(data));return rt.File.writeBytes(target,data);}async function appendFile(path,data,options=undefined){const e=encoding(options);const target=sloppyPath(path);if(typeof data==="string"||e==="utf8"||e==="utf-8")return rt.File.appendText(target,String(data));return rt.File.appendBytes(target,data);}async function access(path){const stat=await rt.File.stat(sloppyPath(path));if(!stat.exists)throw new Error(`SLOPPY_E_NODE_FS_ACCESS: ${path} is not accessible.`);}async function readdir(path){return(await rt.Directory.list(sloppyPath(path))).map((entry)=>entry.name);}async function rm(path,options=undefined){const target=sloppyPath(path);const stat=await rt.File.stat(target);if(stat.kind==="directory")return rt.Directory.delete(target,{recursive:Boolean(options?.recursive)});return rt.File.delete(target);}async function unlink(path){return rt.File.delete(sloppyPath(path));}const api={access,appendFile,mkdir:(path,options)=>rt.Directory.create(sloppyPath(path),{recursive:Boolean(options?.recursive)}),readdir,readFile,rm,stat:(path)=>rt.File.stat(sloppyPath(path)),unlink,writeFile};module.exports={...api,default:api};"#;
...
const NODE_PROCESS_SHIM: &str = r#"const rt=globalThis.__sloppy_runtime||{};const env=new Proxy(Object.create(null),{get(_target,key){return typeof key==="string"?rt.Environment?.get(key):undefined;},ownKeys(){return rt.Environment?.list?.()??[];},getOwnPropertyDescriptor(_target,key){const value=typeof key==="string"?rt.Environment?.get(key):undefined;return value===undefined?undefined:{enumerable:true,configurable:true,value};}});let exitCode=undefined;const process={env,get argv(){return Array.isArray(globalThis.__sloppy_program_args)?globalThis.__sloppy_program_args.slice():[];},cwd(){return globalThis.__sloppy_program_context?.cwd??".";},get platform(){return rt.System?.platform??"sloppy";},get arch(){return rt.System?.arch??"unknown";},version:"sloppy-0.1.0",versions:Object.freeze({sloppy:"0.1.0"}),get exitCode(){return exitCode;},set exitCode(value){exitCode=value;},nextTick(fn,...args){Promise.resolve().then(()=>fn(...args));},exit(code=0){throw new Error(`SLOPPY_E_PROCESS_EXIT_UNSUPPORTED: process.exit(${code}) is not supported.`);}};module.exports=process;module.exports.default=process;"#;

const NODE_CRYPTO_SHIM: &str = r#"const rt=globalThis.__sloppy_runtime||{};function alg(value,op){const normalized=String(value).toLowerCase().replace(/-/g,"");if(["sha256","sha384","sha512"].includes(normalized))return normalized;throw new TypeError(`SLOPPY_E_NODE_CRYPTO_UNSUPPORTED: node:crypto.${op} only supports sha256, sha384, and sha512.`);}function randomBytes(size){return rt.Random.bytes(size);}function randomUUID(){return rt.Random.uuid();}function createHash(algorithm){return rt.Hash.create(alg(algorithm,"createHash"));}function createHmac(algorithm,key){const normalized=alg(algorithm,"createHmac");if(normalized!=="sha256")throw new TypeError("SLOPPY_E_NODE_CRYPTO_UNSUPPORTED: node:crypto.createHmac currently supports sha256.");const chunks=[];return{update(value){chunks.push(value);return this;},async digest(encoding=undefined){const text=chunks.map((chunk)=>typeof chunk==="string"?chunk:String(chunk)).join("");const bytes=await rt.Hmac.sha256(rt.Secret.fromUtf8(String(key)),text);if(encoding==="hex")return rt.Hex.encode(bytes);if(encoding!==undefined&&encoding!=="bytes")throw new TypeError("Sloppy node:crypto Hmac digest encoding must be bytes or hex.");return bytes;}};}function timingSafeEqual(left,right){if(left.byteLength!==right.byteLength)throw new RangeError("Input buffers must have the same byte length.");return rt.ConstantTime.equals(left,right);}module.exports={createHash,createHmac,randomBytes,randomUUID,timingSafeEqual,default:null};module.exports.default=module.exports;"#;

const NODE_ASSERT_SHIM: &str = r#"class AssertionError extends Error{constructor(options={}){super(options.message??"Assertion failed");this.name="AssertionError";this.actual=options.actual;this.expected=options.expected;this.operator=options.operator;this.code="ERR_ASSERTION";}}function fail(options){throw new AssertionError(options);}function ok(value,message=undefined){if(!value)fail({actual:value,expected:true,operator:"ok",message});}function equal(actual,expected,message=undefined){if(actual!=expected)fail({actual,expected,operator:"==",message});}function strictEqual(actual,expected,message=undefined){if(!Object.is(actual,expected))fail({actual,expected,operator:"strictEqual",message});}function stable(value){if(value===null||typeof value!=="object")return JSON.stringify(value);if(Array.isArray(value))return`[${value.map(stable).join(",")}]`;return`{${Object.keys(value).sort().map((key)=>`${JSON.stringify(key)}:${stable(value[key])}`).join(",")}}`;}function deepStrictEqual(actual,expected,message=undefined){if(stable(actual)!==stable(expected))fail({actual,expected,operator:"deepStrictEqual",message});}const deepEqual=deepStrictEqual;function matchExpected(error,expected){if(expected===undefined)return true;if(typeof expected==="function")return error instanceof expected||expected(error)===true;if(expected instanceof RegExp)return expected.test(String(error?.message??error));if(expected!==null&&typeof expected==="object")return Object.entries(expected).every(([key,value])=>Object.is(error?.[key],value));return false;}function throws(fn,expected=undefined,message=undefined){if(typeof fn!=="function")throw new TypeError("assert.throws expects a function.");try{fn();}catch(error){if(!matchExpected(error,expected))throw error;return error;}fail({actual:undefined,expected,operator:"throws",message});}async function rejects(fn,expected=undefined,message=undefined){try{await(typeof fn==="function"?fn():fn);}catch(error){if(!matchExpected(error,expected))throw error;return error;}fail({actual:undefined,expected,operator:"rejects",message});}module.exports={AssertionError,deepEqual,deepStrictEqual,equal,ok,rejects,strictEqual,throws,default:null};module.exports.default=module.exports;"#;

const NODE_STREAM_SHIM: &str = r#"const {EventEmitter}=require("sloppy/node/events");class Stream extends EventEmitter{}class Readable extends Stream{static from(iterable){const readable=new Readable();readable._iterable=iterable;return readable;}async *[Symbol.asyncIterator](){for await(const chunk of this._iterable??[])yield chunk;}}class Writable extends Stream{constructor(options={}){super();this._write=typeof options.write==="function"?options.write:undefined;this.chunks=[];}write(chunk,callback=undefined){Promise.resolve(this._write?this._write(chunk):this.chunks.push(chunk)).then(()=>{if(typeof callback==="function")callback();this.emit("drain");},(error)=>{if(typeof callback==="function")callback(error);this.emit("error",error);});return true;}end(chunk=undefined,callback=undefined){if(chunk!==undefined)this.write(chunk);if(typeof callback==="function")this.once("finish",callback);this.emit("finish");return this;}}class PassThrough extends Writable{write(chunk,callback=undefined){super.write(chunk,callback);this.emit("data",chunk);return true;}}async function pipeline(source,destination,callback=undefined){try{for await(const chunk of source)destination.write(chunk);destination.end();if(typeof callback==="function")callback();return destination;}catch(error){if(typeof callback==="function"){callback(error);return destination;}throw error;}}module.exports={PassThrough,Readable,Stream,Writable,pipeline,default:null};module.exports.default=module.exports;"#;
...
fn program_import_replacement(
    ...
    for default in defaults {
        ...
        output.push_str(&require_expr);
        output.push_str(";return __sloppy_default_module&&Object.prototype.hasOwnProperty.call(__sloppy_default_module,\"default\")?__sloppy_default_module.default:__sloppy_default_module;})();");
    }
