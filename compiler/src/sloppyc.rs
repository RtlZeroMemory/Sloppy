...
fn transform_program_source(
    ...
    if format == ModuleFormat::Json {
        ...
        return Ok(format!(
            "const __sloppy_json_module = {}; module.exports = Object(__sloppy_json_module) === __sloppy_json_module ? __sloppy_json_module : {{ default: __sloppy_json_module }}; Object.defineProperty(module.exports,\"__esModule\",{{value:true}}); module.exports.default = __sloppy_json_module;\n",
...
const NODE_FS_SHIM: &str = r#"const promises=require("sloppy/node/fs/promises");function unsupported(name){return function(){throw new Error(`node:fs.${name} is not implemented by Sloppy's node:fs compatibility shim. Use sloppy/fs when available, or avoid this package path.`);};}function callbackify(method,name){return function(){const args=Array.prototype.slice.call(arguments);const callback=args.pop();if(typeof callback!=="function")throw new TypeError(`node:fs.${name} requires a callback.`);method(...args).then((value)=>callback(null,value),(error)=>callback(error));};}const api={access:callbackify(promises.access,"access"),appendFile:callbackify(promises.appendFile,"appendFile"),existsSync:unsupported("existsSync"),mkdir:callbackify(promises.mkdir,"mkdir"),promises,readdir:callbackify(promises.readdir,"readdir"),readFile:callbackify(promises.readFile,"readFile"),rm:callbackify(promises.rm,"rm"),stat:callbackify(promises.stat,"stat"),unlink:callbackify(promises.unlink,"unlink"),watch:unsupported("watch"),writeFile:callbackify(promises.writeFile,"writeFile"),default:null};module.exports=api;module.exports.default=api;"#;

const NODE_FS_PROMISES_SHIM: &str = r#"const rt=globalThis.__sloppy_runtime||{};function encoding(options,op){const e=typeof options==="string"?options:options?.encoding;if(e===undefined||e===null)return undefined;const n=String(e).toLowerCase();if(n==="utf8"||n==="utf-8")return"utf8";throw new TypeError(`SLOPPY_E_NODE_FS_UNSUPPORTED_ENCODING: ${op} only supports utf8 text encoding.`);}function sloppyPath(path){if(typeof path!=="string")return path;const value=path.replace(/\\/g,"/");if(value.startsWith("/")||/^[A-Za-z]:\//.test(value)||value.startsWith("./")||value.startsWith("../")||value.includes("://"))return value;return `./${value}`;}function stats(stat){return Object.freeze({...stat,isFile(){return stat.kind==="file"||(stat.exists===true&&stat.kind!=="directory");},isDirectory(){return stat.kind==="directory";}});}async function readFile(path,options=undefined){const e=encoding(options,"node:fs.readFile");const target=sloppyPath(path);return e==="utf8"?rt.File.readText(target):rt.File.readBytes(target);}async function writeFile(path,data,options=undefined){const e=encoding(options,"node:fs.writeFile");const target=sloppyPath(path);if(typeof data==="string"||e==="utf8")return rt.File.writeText(target,String(data));return rt.File.writeBytes(target,data);}async function appendFile(path,data,options=undefined){const e=encoding(options,"node:fs.appendFile");const target=sloppyPath(path);if(typeof data==="string"||e==="utf8")return rt.File.appendText(target,String(data));return rt.File.appendBytes(target,data);}async function access(path){const stat=await rt.File.stat(sloppyPath(path));if(!stat.exists)throw new Error(`SLOPPY_E_NODE_FS_ACCESS: ${path} is not accessible.`);}async function readdir(path){return(await rt.Directory.list(sloppyPath(path))).map((entry)=>entry.name);}async function rm(path,options=undefined){const target=sloppyPath(path);const stat=await rt.File.stat(target);if(stat.kind==="directory")return rt.Directory.delete(target,{recursive:Boolean(options?.recursive)});return rt.File.delete(target);}async function unlink(path){return rt.File.delete(sloppyPath(path));}const api={access,appendFile,mkdir:(path,options)=>rt.Directory.create(sloppyPath(path),{recursive:Boolean(options?.recursive)}),readdir,readFile,rm,stat:async(path)=>stats(await rt.File.stat(sloppyPath(path))),unlink,writeFile};module.exports={...api,default:api};"#;
