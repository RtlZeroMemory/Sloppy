...
fn ensure_node_compat_module(
    ...
    if backing == "sloppy/node/https" {
        ensure_node_compat_module("sloppy/node/http", graph, modules);
    }
...
const NODE_HTTP_SHIM: &str = r#"class EventEmitter{constructor(){this._events=Object.create(null);}on(name,fn){if(typeof fn!=="function")throw new TypeError("listener must be a function");(this._events[name]??=[]).push(fn);return this;}once(name,fn){const wrapped=(...args)=>{this.off(name,wrapped);fn(...args);};wrapped._original=fn;return this.on(name,wrapped);}off(name,fn){const list=this._events[name]||[];this._events[name]=list.filter((item)=>item!==fn&&item._original!==fn);return this;}removeListener(name,fn){return this.off(name,fn);}emit(name,...args){const list=(this._events[name]||[]).slice();for(const fn of list)fn(...args);return list.length>0;}}const rt=globalThis.__sloppy_runtime||{};function bytes(value,encoding="utf8"){if(value===undefined||value===null)return new Uint8Array(0);if(typeof value==="string"){if(encoding!=="utf8"&&encoding!=="utf-8")throw new TypeError("SLOPPY_E_NODE_HTTP_UNSUPPORTED: node:http request bodies only support utf8 string encoding.");return rt.Text?.utf8?.encode?rt.Text.utf8.encode(value):new TextEncoder().encode(value);}if(value instanceof Uint8Array)return value;if(value instanceof ArrayBuffer)return new Uint8Array(value);if(ArrayBuffer.isView(value))return new Uint8Array(value.buffer,value.byteOffset,value.byteLength);throw new TypeError("ERR_INVALID_ARG_TYPE: node:http request body must be a string or bytes.");}function concat(chunks){const length=chunks.reduce((total,chunk)=>total+chunk.byteLength,0);const out=new Uint8Array(length);let offset=0;for(const chunk of chunks){out.set(chunk,offset);offset+=chunk.byteLength;}return out;}function headerEntries(headers){return typeof headers?.entries==="function"?headers.entries():Object.entries(headers||{});}function headerObject(headers){const out=Object.create(null);for(const [name,value]of headerEntries(headers)){const key=String(name).toLowerCase();out[key]=out[key]===undefined?String(value):`${out[key]}, ${value}`;}return out;}function rawHeaders(headers){const out=[];for(const [name,value]of headerEntries(headers)){out.push(String(name),String(value));}return out;}function normalize(protocol,input,options=undefined){let url;let opts={};if(input instanceof URL)url=new URL(input.href);else if(typeof input==="string")url=new URL(input);else if(input&&typeof input==="object")opts=input;else throw new TypeError("ERR_INVALID_ARG_TYPE: node:http.request requires a URL string, URL, or options object.");if(typeof options==="string")opts={...opts,path:options};else if(options&&typeof options==="object")opts={...opts,...options};if(url===undefined){const scheme=opts.protocol??protocol;const host=opts.hostname??opts.host??"localhost";const port=opts.port===undefined?"":`:${opts.port}`;const path=opts.path??"/";url=new URL(`${scheme}//${host}${port}${path}`);}if(url.protocol!==protocol)throw new TypeError(`ERR_INVALID_PROTOCOL: expected ${protocol} request URL.`);const headers={};for(const [name,value]of Object.entries(opts.headers??{}))headers[name]=Array.isArray(value)?value.join(", "):String(value);return{url:url.href,method:String(opts.method??"GET").toUpperCase(),headers,timeoutMs:opts.timeout,signal:opts.signal};}class IncomingMessage extends EventEmitter{constructor(response,body){super();this.statusCode=response.status;this.statusMessage=response.statusText;this.headers=headerObject(response.headers);this.rawHeaders=rawHeaders(response.headers);this._body=body;this.readableEnded=false;}_start(){Promise.resolve().then(()=>{if(this._body.byteLength>0)this.emit("data",this._body);this.readableEnded=true;this.emit("end");this.emit("close");});}async *[Symbol.asyncIterator](){if(this._body.byteLength>0)yield this._body;}}class ClientRequest extends EventEmitter{constructor(protocol,input,options,callback){super();this._request=normalize(protocol,input,options);this._headers=new Map(Object.entries(this._request.headers).map(([name,value])=>[name.toLowerCase(),{name,value}]));this._chunks=[];this.destroyed=false;this.writableEnded=false;if(typeof callback==="function")this.once("response",callback);}setHeader(name,value){this._headers.set(String(name).toLowerCase(),{name:String(name),value:String(value)});return this;}getHeader(name){return this._headers.get(String(name).toLowerCase())?.value;}removeHeader(name){this._headers.delete(String(name).toLowerCase());}write(chunk,encodingOrCallback=undefined,callback=undefined){if(this.destroyed||this.writableEnded)throw new Error("ERR_STREAM_WRITE_AFTER_END: cannot write after request end.");const encoding=typeof encodingOrCallback==="string"?encodingOrCallback:"utf8";this._chunks.push(bytes(chunk,encoding));const done=typeof encodingOrCallback==="function"?encodingOrCallback:callback;if(typeof done==="function")Promise.resolve().then(done);return true;}end(chunk=undefined,encodingOrCallback=undefined,callback=undefined){if(typeof chunk==="function"){callback=chunk;chunk=undefined;}else if(typeof encodingOrCallback==="function")callback=encodingOrCallback;if(chunk!==undefined)this.write(chunk,typeof encodingOrCallback==="string"?encodingOrCallback:"utf8");if(typeof callback==="function")this.once("finish",callback);this.writableEnded=true;this.emit("finish");this._send();return this;}destroy(error=undefined){this.destroyed=true;if(error!==undefined)this.emit("error",error);this.emit("close");return this;}abort(){return this.destroy();}async _send(){if(this.destroyed)return;try{if(typeof rt.HttpClient?.request!=="function")throw new Error("SLOPPY_E_NODE_HTTP_UNSUPPORTED: node:http requires the Sloppy HttpClient runtime feature.");const headers={};for(const {name,value}of this._headers.values())headers[name]=value;const response=await rt.HttpClient.request(this._request.url,{method:this._request.method,headers,bytes:concat(this._chunks),redirects:false,signal:this._request.signal,timeoutMs:this._request.timeoutMs});const body=await response.bytes();if(this.destroyed)return;const message=new IncomingMessage(response,body);this.emit("response",message);message._start();this.emit("close");}catch(error){if(!this.destroyed){this.emit("error",error);this.emit("close");}}}}class Agent{constructor(options=undefined){this.options=Object.freeze({...(options??{})});}destroy(){}}const globalAgent=new Agent({keepAlive:false});function request(input,options=undefined,callback=undefined){if(typeof options==="function"){callback=options;options=undefined;}return new ClientRequest("http:",input,options,callback);}function get(input,options=undefined,callback=undefined){const req=request(input,options,callback);req.end();return req;}module.exports={Agent,ClientRequest,IncomingMessage,request,get,globalAgent,default:null};module.exports.default=module.exports;"#;
const NODE_HTTPS_SHIM: &str = r#"const http=require("sloppy/node/http");const globalAgent=new http.Agent({keepAlive:false,protocol:"https:"});function request(input,options=undefined,callback=undefined){if(typeof options==="function"){callback=options;options=undefined;}const merged=options&&typeof options==="object"?{...options,protocol:"https:"}:{protocol:"https:"};return new http.ClientRequest("https:",input,merged,callback);}function get(input,options=undefined,callback=undefined){const req=request(input,options,callback);req.end();return req;}module.exports={Agent:http.Agent,ClientRequest:http.ClientRequest,IncomingMessage:http.IncomingMessage,request,get,globalAgent,default:null};module.exports.default=module.exports;"#;
