...
pub(crate) const FRAMEWORK_SERVICE_RUNTIME: &str = r#"const __sloppy_framework_services = (() => {
...
fn wrapper_source(handler_source: &str, bindings: &[RequestBinding]) -> String {
    let args = bindings
        ...
        .join(", ");
    format!(
        "async (ctx) => {{ const __sloppy_scope = __sloppy_framework_services.createScope(ctx); ctx.services = __sloppy_scope; try {{ const __sloppy_typed_handler = {handler_source}; return await __sloppy_typed_handler({args}); }} finally {{ await __sloppy_scope.dispose(); }} }}"
    )
}
