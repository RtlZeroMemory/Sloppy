 ...
 *   dependency, route dispatch, body parser, query parser, or percent decoder.
 *
                                  ...
                                  SlStr hint)
{
    ...
    ctx->diag_hint = hint;
    return sl_status_from_code(code == SL_DIAG_HTTP_HEADER_LIMIT ? SL_STATUS_CAPACITY_EXCEEDED
                                          ...
                                          llhttp_t* out_parser)
{
    ...
    if (parse_error != HPE_OK) {
        if (ctx->diag_code != SL_DIAG_NONE) {
            return sl_status_from_code(ctx->diag_code == SL_DIAG_HTTP_HEADER_LIMIT
