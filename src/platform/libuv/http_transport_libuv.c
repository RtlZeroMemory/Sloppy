                                                   ...
                                                   SlHttpTransportConfig* out, SlDiag* out_diag)
{
    ...
    if (input != NULL) {
        ...
        config.websocket_upgrade = input->websocket_upgrade;
        config.websocket_upgrade_user = input->websocket_upgrade_user;
        config.websocket_open = input->websocket_open;
        config.websocket_open_user = input->websocket_open_user;
        config.websocket_frame = input->websocket_frame;
        config.websocket_frame_user = input->websocket_frame_user;
    }
...
static void sl_http_transport_connection_close_cb(uv_handle_t* handle)
{
    ...
    sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_CONNECTIONS_CLOSED, 1U);
}
...
static void sl_http_transport_reset_request_state(SlHttpTransportConnection* connection)
{
    ...
    if (connection->body_reader_started && !connection->body_reader_finished) {
        sl_http_request_body_reader_close(&connection->body_reader, NULL);
    }
    if (connection->request_started) {
        sl_http_request_close(&connection->request, NULL);
    }
    sl_arena_reset(&connection->request_arena);
    sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_ARENA_RESETS, 1U);
    sl_byte_builder_init_fixed(&connection->accumulation_builder, connection->accumulation,
    ...
    connection->request_content_type = sl_str_empty();
    connection->request_started = false;
    connection->request_is_chunked = false;
    ...
    connection->websocket_upgraded = false;
    connection->websocket_selected_protocol = sl_str_empty();
    connection->active_response = (SlHttpResponse){0};
    connection->stream_readable_adapter = (SlHttpResponseStreamReadable){0};
    connection->stream_readable = (SlReadableStream){0};
    connection->stream_chunks_written = 0U;
    connection->stream_final_written = false;
    connection->websocket_message_count = 0U;
    connection->websocket_user_state = NULL;
}

static void sl_http_transport_begin_websocket_frame_mode(SlHttpTransportConnection* connection)
{
    if (connection == NULL) {
        return;
    }
                                                    ...
                                                    SlBytes bytes, SlDiag* out_diag)
{
    ...
    if (!connection->platform->tls_enabled) {
        ...
        sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_UV_TRY_WRITE_CALLS, 1U);
        rc = uv_try_write((uv_stream_t*)&connection->platform->handle, &buffer, 1U);
        ...
        if (rc > 0) {
            buffer = uv_buf_init((char*)bytes.ptr + rc, (unsigned int)(bytes.length - (size_t)rc));
        }
    ...
    {
        uint64_t started_ns = sl_http_profile_now_ns();
        sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_UV_WRITE_CALLS, 1U);
                                                    ...
                                                    bool* out_handled, SlDiag* out_diag)
{
                                                                         ...
                                                                         : SL_HTTP2_PREFACE_BYTES)))
    {
        ...
        {
            *out_handled = true;
            sl_http_transport_connection_diag(
                connection, out_diag, SL_DIAG_INVALID_HTTP_REQUEST, SL_STATUS_INVALID_ARGUMENT,
                                                    ...
                                                    SlDiag* out_diag)
{
    ...
    stream_status = sl_readable_stream_read(&connection->stream_readable, &read);
    if (stream_status == SL_STREAM_STATUS_OK) {
        SlBytes chunk = read.chunk.bytes;
        SlHttpTransportServer* server = sl_http_transport_connection_server(connection);
        if (server == NULL) {
            return sl_http_transport_connection_diag(
        ...
        connection->stream_chunks_written += 1U;
    }
    else if (stream_status == SL_STREAM_STATUS_EOF) {
        ...
        return sl_http_transport_connection_diag(
            connection, out_diag, SL_DIAG_HTTP_RESPONSE_SERIALIZATION_FAILED,
            sl_status_code(sl_stream_status_to_status(stream_status)),
            sl_http_transport_literal("HTTP streaming response read failed",
                                      sizeof("HTTP streaming response read failed") - 1U),
            sl_http_transport_literal("stream failures are mapped before writing body bytes",
                                      sizeof("stream failures are mapped before writing body "
                                             "bytes") -
                                          1U));
    }
    if (!sl_status_is_ok(status)) {
    ...
    {
        SlHttpTransportServer* server = sl_http_transport_connection_server(connection);
        size_t next_response_length = 0U;
        status =
            sl_checked_add_size(connection->response_length, bytes.length, &next_response_length);
        if (server == NULL || !sl_status_is_ok(status) ||
            next_response_length > server->config.max_response_bytes)
        {
            return sl_http_transport_connection_diag(
                connection, out_diag, SL_DIAG_HTTP_RESPONSE_BACKPRESSURE,
                SL_STATUS_CAPACITY_EXCEEDED,
                sl_http_transport_literal("HTTP streaming response exceeded response cap",
                                          sizeof("HTTP streaming response exceeded response "
                                                 "cap") -
                                              1U),
                sl_http_transport_literal("stream frames must fit the configured response cap",
                                          sizeof("stream frames must fit the configured response "
                                                 "cap") -
                                              1U));
        }
        connection->response_length = next_response_length;
    }
    return sl_http_transport_start_write_bytes(connection, bytes, out_diag);
}
                                                       ...
                                                       SlHttpResponse* out_response)
{
    ...
    *out_response = *response;
...

    {
        ...
        status = sl_checked_array_size(response->stream_chunk_count, sizeof(SlStreamChunk),
                                       &chunk_bytes);
        if (!sl_status_is_ok(status)) {
        ...
        status = sl_arena_alloc(&connection->request_arena, chunk_bytes, _Alignof(SlStreamChunk),
                                &chunk_storage);
    }
    if (!sl_status_is_ok(status)) {
        *out_response = (SlHttpResponse){0};
    ...
    chunks = (SlStreamChunk*)chunk_storage;
    options.max_chunk_bytes = (size_t)-1;
    options.max_buffered_bytes =
        sl_http_transport_connection_server(connection) == NULL
            ? SL_STREAM_DEFAULT_MAX_BUFFERED_BYTES
            : sl_http_transport_connection_server(connection)->config.max_response_bytes;
    options.max_chunks = response->stream_chunk_count;
    status =
        sl_http_response_stream_readable_init(&readable_adapter, response, &options, &readable);
    if (!sl_status_is_ok(status)) {
        *out_response = (SlHttpResponse){0};
        return status;
    }
    ...
    for (;;) {
        ...
        if (!sl_http_transport_arena_contains(&connection->request_arena, read.chunk.bytes.ptr,
                                              read.chunk.bytes.length))
        {
            *out_response = (SlHttpResponse){0};
            return sl_status_from_code(SL_STATUS_INVALID_ARGUMENT);
        }
        if (copied_count >= response->stream_chunk_count) {
            *out_response = (SlHttpResponse){0};
            return sl_status_from_code(SL_STATUS_CAPACITY_EXCEEDED);
        }
        status = sl_bytes_copy_to_arena(&connection->request_arena, read.chunk.bytes, &copy);
        if (!sl_status_is_ok(status)) {
            *out_response = (SlHttpResponse){0};
            return status;
        }
        chunks[copied_count].bytes = sl_owned_bytes_as_view(copy);
        copied_count += 1U;
    }
    out_response->stream_chunks = chunks;
    out_response->stream_chunk_count = copied_count;
    return sl_status_ok();
}
                                                    ...
                                                    SlDiag* out_diag)
{
    ...
    SlStreamOptions stream_options = sl_stream_default_options();
    ...
    stream_options.max_chunk_bytes =
        sl_http_transport_connection_server(connection) == NULL
            ? SL_STREAM_DEFAULT_MAX_CHUNK_BYTES
            : sl_http_transport_connection_server(connection)->config.max_pending_write_bytes;
    stream_options.max_buffered_bytes =
        sl_http_transport_connection_server(connection) == NULL
            ? SL_STREAM_DEFAULT_MAX_BUFFERED_BYTES
            : sl_http_transport_connection_server(connection)->config.max_response_bytes;
    stream_options.max_chunks =
        response_copy.stream_chunk_count == 0U ? 1U : response_copy.stream_chunk_count;
    status = sl_http_response_stream_readable_init(&connection->stream_readable_adapter,
                                                   &connection->active_response, &stream_options,
                                                   &connection->stream_readable);
    if (!sl_status_is_ok(status)) {
        return sl_http_transport_connection_diag(
            connection, out_diag, SL_DIAG_HTTP_RESPONSE_SERIALIZATION_FAILED,
            sl_status_code(status),
            sl_http_transport_literal("HTTP streaming response readable init failed",
                                      sizeof("HTTP streaming response readable init failed") - 1U),
            sl_http_transport_literal("stream descriptors must be valid before transport write",
                                      sizeof("stream descriptors must be valid before transport "
                                             "write") -
                                          1U));
    }
    ...
    connection->stream_final_written = false;
    {
        ...
        if (bytes.length > server->config.max_response_bytes) {
            return sl_http_transport_connection_diag(
                connection, out_diag, SL_DIAG_HTTP_RESPONSE_BACKPRESSURE,
                SL_STATUS_CAPACITY_EXCEEDED,
                sl_http_transport_literal("HTTP streaming response exceeded response cap",
                                          sizeof("HTTP streaming response exceeded response "
                                                 "cap") -
                                              1U),
                sl_http_transport_literal("stream response head must fit the configured response "
                                          "cap",
                                          sizeof("stream response head must fit the configured "
                                                 "response cap") -
                                              1U));
...
static void sl_http_transport_complete_write(SlHttpPlatformConnection* platform, int status)
{
    ...
    SlDiag diag = {0};
    uint64_t profile_started_ns = sl_http_profile_now_ns();
    ...
    sl_http_profile_record_phase(SL_HTTP_PROFILE_PHASE_WRITE_COMPLETION,
                                 sl_http_profile_now_ns() - profile_started_ns);
    ...
    if (connection->websocket_upgraded) {
        SlHttpTransportServer* server = sl_http_transport_connection_server(connection);
        bool completed_upgrade_request = false;
        connection->write_completed = true;
        if (connection->request_started &&
            !sl_http_transport_request_terminal(connection->request.state))
        {
            if (status != 0) {
                sl_http_connection_fail(&connection->core, NULL);
                sl_http_request_fail(&connection->request, NULL);
                sl_http_transport_connection_close(connection, NULL);
                return;
            }
            sl_http_request_complete(&connection->request, NULL);
            completed_upgrade_request = true;
        }
        if (status != 0 || connection->close_after_write ||
            connection->state == SL_HTTP_TRANSPORT_CONNECTION_STATE_CLOSING)
        {
            sl_http_transport_connection_close(connection, NULL);
            return;
        ...
        if (completed_upgrade_request) {
            ...
            if (server != NULL && server->config.websocket_open != NULL) {
                ...
                if (connection->state == SL_HTTP_TRANSPORT_CONNECTION_STATE_WRITING_RESPONSE) {
                    return;
                }
            }
            sl_http_transport_begin_websocket_frame_mode(connection);
        }
        connection->state = SL_HTTP_TRANSPORT_CONNECTION_STATE_UPGRADED;
        return;
    }
    ...
    if (status == 0 && connection->keep_alive_after_write && !connection->close_after_write) {
        ...
        sl_http_profile_flush_if_requested();
                                                 ...
                                                 const SlHttpResponse* response, SlDiag* out_diag)
{
    ...
    if (response->kind == SL_HTTP_RESPONSE_STREAM) {
        if (!suppress_body && response->status != 204U && response->status != 304U) {
...

    {
        ...
        sl_http_profile_record_phase(SL_HTTP_PROFILE_PHASE_RESPONSE_SERIALIZATION,
                                     sl_http_profile_now_ns() - started_ns);
    }
    if (!sl_status_is_ok(status)) {
        return sl_http_transport_connection_diag(
    ...
    connection->response_length = bytes.length;
    sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_RESPONSE_BYTES_WRITTEN, bytes.length);
    sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_RESPONSE_BUFFER_COPIES, 1U);
    return sl_http_transport_start_write_bytes(connection, bytes, out_diag);
}
...
static void sl_http_transport_timeout_connection(SlHttpTransportConnection* connection, SlStr phase)
{
    ...
    if (connection->platform != NULL && connection->platform->reading) {
        uv_read_stop((uv_stream_t*)&connection->platform->handle);
    ...
    {
        sl_http_request_timeout(&connection->request, phase, NULL);
    }
                                                          ...
                                                          SlDiag* out_diag)
{
    ...
    sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_KEEP_ALIVE_REUSED, 1U);
    {
        uint64_t started_ns = sl_http_profile_now_ns();
        sl_http_transport_reset_request_state(connection);
        sl_http_profile_record_phase(SL_HTTP_PROFILE_PHASE_CONNECTION_KEEP_ALIVE,
                                     sl_http_profile_now_ns() - started_ns);
    }
                                                     ...
                                                     SlStr* out_settings, bool* out_malformed)
{
    ...
    sl_http_transport_request_header_value_count(head, sl_str_from_cstr("HTTP2-Settings"),
...
static bool sl_http_transport_request_is_websocket_upgrade(SlHttpTransportConnection* connection)
{
    SlHttpRequestHead* head = connection == NULL ? NULL : &connection->request.head;
    SlStr upgrade = {0};
    ...
    if (!sl_http_transport_header_value(head, sl_str_from_cstr("Upgrade"), &upgrade)) {
        return false;
    }
    return sl_str_equal_ci_ascii(sl_http_transport_trim_ascii_space(upgrade),
                                 sl_str_from_cstr("websocket"));
}

static SlStatus
sl_http_transport_write_websocket_handshake(SlHttpTransportConnection* connection,
                                            const SlWebSocketHandshakeResult* result,
                                            SlDiag* out_diag)
{
    SlHttpTransportServer* server = sl_http_transport_connection_server(connection);
    ...
    if (result->http_status != 101U) {
        return sl_http_transport_write_error_response(connection, result->http_status, out_diag);
    }
    if (connection->request.state == SL_HTTP_REQUEST_STATE_DISPATCHING) {
        SlStatus status = sl_http_request_begin_write(&connection->request, out_diag);
        if (!sl_status_is_ok(status)) {
            sl_http_transport_store_diag(connection, out_diag);
            return status;
        }
    }
    if (result->response_bytes.ptr == NULL || result->response_bytes.length == 0U) {
        return sl_http_transport_connection_diag(
            connection, out_diag, SL_DIAG_HTTP_RESPONSE_SERIALIZATION_FAILED,
            SL_STATUS_INVALID_ARGUMENT,
            sl_http_transport_literal("WebSocket handshake response is empty",
                                      sizeof("WebSocket handshake response is empty") - 1U),
            sl_http_transport_literal("accepted upgrades must produce a 101 response",
                                      sizeof("accepted upgrades must produce a 101 response") -
                                          1U));
    }
    if (result->response_bytes.length > UINT_MAX ||
        ...
        return sl_http_transport_connection_diag(
            ...
            sl_http_transport_literal("WebSocket handshake exceeded pending write cap",
                                      sizeof("WebSocket handshake exceeded pending write cap") -
                                          1U),
            sl_http_transport_literal("101 response bytes must fit the pending write cap",
                                      sizeof("101 response bytes must fit the pending write cap") -
                                          1U));
    }

    connection->response_length = result->response_bytes.length;
    connection->state = SL_HTTP_TRANSPORT_CONNECTION_STATE_WRITING_RESPONSE;
    connection->websocket_upgraded = true;
    connection->websocket_selected_protocol = result->selected_protocol;
    connection->keep_alive_after_write = false;
    connection->close_after_write = false;
    return sl_http_transport_start_write_bytes(connection, result->response_bytes, out_diag);
}

static SlStatus sl_http_transport_try_websocket_upgrade(SlHttpTransportConnection* connection,
                                                        bool* out_upgraded, SlDiag* out_diag)
{
    SlHttpTransportServer* server = sl_http_transport_connection_server(connection);
    SlWebSocketHandshakeResult result = {0};
    SlDiag upgrade_diag = {0};
    SlStatus status;

    if (out_upgraded != NULL) {
        *out_upgraded = false;
    }
    if (connection == NULL || server == NULL || out_upgraded == NULL) {
        return sl_status_from_code(SL_STATUS_INVALID_ARGUMENT);
    }
    if (!sl_http_transport_request_is_websocket_upgrade(connection)) {
        return sl_status_ok();
    }
    *out_upgraded = true;
    ...
    if (server->config.websocket_upgrade == NULL) {
        sl_http_transport_connection_diag(
            connection, out_diag, SL_DIAG_HTTP_UNSUPPORTED_BODY, SL_STATUS_UNSUPPORTED,
            sl_http_transport_literal("WebSocket Upgrade is not configured",
                                      sizeof("WebSocket Upgrade is not configured") - 1U),
            sl_http_transport_literal("configure a WebSocket upgrade callback before listening",
                                      sizeof("configure a WebSocket upgrade callback before "
                                             "listening") -
                                          1U));
        return sl_http_transport_write_error_response(connection, 501U, out_diag);
    }

    status = server->config.websocket_upgrade(connection, &connection->request_arena,
                                              &connection->request, &result, &upgrade_diag,
                                              server->config.websocket_upgrade_user);
    if (!sl_status_is_ok(status)) {
        uint16_t safe_status = sl_http_transport_status_for_failure(status, &upgrade_diag);
        sl_http_transport_store_diag(connection, &upgrade_diag);
        result.http_status = safe_status;
    }
    else if (result.http_status == 0U) {
        result.http_status = 500U;
    }
    if (result.http_status != 101U && upgrade_diag.code != SL_DIAG_NONE) {
        sl_http_transport_store_diag(connection, &upgrade_diag);
    }
    return sl_http_transport_write_websocket_handshake(connection, &result, out_diag);
}
...
static void sl_http_transport_dispatch_check_cb(uv_check_t* check)
{
    ...
    sl_http_transport_dispatch_ready_connections(server, &diag, false);
}
                                                    ...
                                                    SlDiag* out_diag)
{
    ...
    full_message = sl_bytes_from_parts(connection->accumulation, connection->accumulation_length);
    {
        ...
        sl_http_profile_record_phase(SL_HTTP_PROFILE_PHASE_HTTP_PARSE,
                                     sl_http_profile_now_ns() - started_ns);
    }
    if (!sl_status_is_ok(status)) {
        if (out_diag != NULL) {
    ...
    {
        uint64_t started_ns = sl_http_profile_now_ns();
        is_chunked = connection->request_is_chunked;
        content_type = connection->request_content_type;
        sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_HEADER_COUNT,
                              connection->request.head.header_count);
        sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_BODY_BYTES,
                              connection->request.head.body.length);
        sl_http_profile_record_phase(SL_HTTP_PROFILE_PHASE_HEADER_CLASSIFICATION,
                                     sl_http_profile_now_ns() - started_ns);
    }
                                                       ...
                                                       SlDiag* out_diag)
{
...

    {
        ...
        SlBytes head_bytes = sl_bytes_from_parts(connection->accumulation, connection->head_length);

        has_transfer_encoding =
            sl_http_transport_head_header_value(head_bytes, sl_str_from_cstr("Transfer-Encoding"),
                                                &transfer_encoding, &transfer_encoding_count);
        has_content_length = sl_http_transport_head_header_value(
            head_bytes, sl_str_from_cstr("Content-Length"), &content_length, &content_length_count);
        has_content_type = sl_http_transport_head_header_value(
            head_bytes, sl_str_from_cstr("Content-Type"), &content_type, NULL);
        connection->request_content_type = has_content_type ? content_type : sl_str_empty();
        connection->request_is_chunked = false;
        if (has_transfer_encoding) {
            ...
            connection->request_is_chunked = true;
        ...
        if (has_content_length) {
            ...
            if (content_length_count == 1U) {
                parsed = sl_http_transport_parse_size_decimal(content_length,
                                                              &connection->expected_body_length);
            }
            sl_http_profile_record_phase(SL_HTTP_PROFILE_PHASE_MEDIA_TYPE_CONTENT_LENGTH,
                                         sl_http_profile_now_ns() - started_ns);
            if (content_length_count != 1U || !parsed) {
                return sl_http_transport_connection_diag(
                    connection, out_diag, SL_DIAG_INVALID_HTTP_REQUEST, SL_STATUS_INVALID_ARGUMENT,
    ...
    for (index = 0U; index < server->connection_capacity; index += 1U) {
            ...
            connection->state == SL_HTTP_TRANSPORT_CONNECTION_STATE_CLOSED)
        {
            ...
            *connection = (SlHttpTransportConnection){0};
            ...
            sl_arena_init(&connection->request_arena, connection->request_storage,
                                                   ...
                                                   SlDiag* diag, SlStr cancel_message)
{
    ...
    if (connection->request_started && connection->request.state != SL_HTTP_REQUEST_STATE_CLOSED &&
        ...
        connection->request.state != SL_HTTP_REQUEST_STATE_FAILED)
    {
        sl_http_request_cancel(&connection->request, SL_CANCELLATION_REASON_CANCELLED,
                               cancel_message, NULL);
    }
    sl_http_transport_connection_close(connection, NULL);
}
...
static void sl_http_transport_on_connection(uv_stream_t* listener, int status)
{
    ...
    sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_CONNECTIONS_OPENED, 1U);
...
SlStatus sl_http_transport_server_listen(SlHttpTransportServer* server, SlDiag* out_diag)
{
    ...
    if (rc != 0) {
        server->state = SL_HTTP_TRANSPORT_SERVER_STATE_ERROR;
        uv_loop_close(&server->platform->loop);
...
SlStatus sl_http_transport_server_poll(SlHttpTransportServer* server, SlDiag* out_diag)
{
    ...
    uv_run(&server->platform->loop, UV_RUN_NOWAIT);
...
SlStatus sl_http_transport_server_run(SlHttpTransportServer* server, SlDiag* out_diag)
{
    ...
    status = sl_http_profile_flush_if_requested();
    return status;
}
...
SlStatus sl_http_transport_connection_close(SlHttpTransportConnection* connection, SlDiag* out_diag)
{
    ...
    if (connection->platform != NULL && connection->platform->writing) {
        ...
        if (connection->request_started &&
            !sl_http_transport_request_terminal(connection->request.state))
        {
            sl_http_request_shutdown(&connection->request, NULL);
        }
        connection->state = SL_HTTP_TRANSPORT_CONNECTION_STATE_CLOSING;
        return sl_status_ok();
    }
    ...
    sl_http_connection_close(&connection->core, out_diag);
    connection->state = SL_HTTP_TRANSPORT_CONNECTION_STATE_CLOSING;
    if (connection->platform != NULL && connection->platform->initialized &&
                                          ...
                                          SlDiag* out_diag)
{
        ...
        connection->state != SL_HTTP_TRANSPORT_CONNECTION_STATE_UPGRADED)
    {
        return sl_http_transport_connection_diag(
            connection, out_diag, SL_DIAG_APP_LIFECYCLE, SL_STATUS_INVALID_STATE,
            sl_http_transport_literal("WebSocket frame write state is invalid",
                                      sizeof("WebSocket frame write state is invalid") - 1U),
            sl_http_transport_literal("send frames only after the HTTP Upgrade is complete",
                                      sizeof("send frames only after the HTTP Upgrade is "
                                             "complete") -
                                          1U));
    }
    status = sl_byte_builder_init_fixed(&builder, connection->response_storage,
    ...
    status = sl_websocket_write_frame(&builder, options, out_diag);
    if (!sl_status_is_ok(status)) {
        sl_http_transport_store_diag(connection, out_diag);
        return status;
    }
    connection->response_length = sl_byte_builder_length(&builder);
    connection->state = SL_HTTP_TRANSPORT_CONNECTION_STATE_WRITING_RESPONSE;
    connection->write_started = true;
    connection->write_completed = false;
    return sl_http_transport_start_write_bytes(
        connection, sl_bytes_from_parts(connection->response_storage, connection->response_length),
        out_diag);
}

static SlStatus sl_http_transport_websocket_send_close(SlHttpTransportConnection* connection,
                                                       uint16_t close_code, SlDiag* out_diag)
{
    unsigned char payload[2];
    SlWebSocketFrameWriteOptions options = {0};

    payload[0] = (unsigned char)((close_code >> 8U) & 0xFFU);
    payload[1] = (unsigned char)(close_code & 0xFFU);
    options.fin = true;
    options.opcode = SL_WEBSOCKET_OPCODE_CLOSE;
    options.payload = sl_bytes_from_parts(payload, sizeof(payload));
    connection->close_after_write = true;
    return sl_http_transport_websocket_send(connection, &options, out_diag);
}

static SlStatus sl_http_transport_websocket_dispatch_frame(SlHttpTransportConnection* connection,
                                                           ...
                                                           SlDiag* out_diag)
{
    ...
    if (connection == NULL || frame == NULL || server == NULL) {
        return sl_status_from_code(SL_STATUS_INVALID_ARGUMENT);
    }
    if (frame->opcode == SL_WEBSOCKET_OPCODE_PING) {
        SlWebSocketFrameWriteOptions options = {0};
        options.fin = true;
        options.opcode = SL_WEBSOCKET_OPCODE_PONG;
        options.payload = frame->payload;
        return sl_http_transport_websocket_send(connection, &options, out_diag);
    }
    if (frame->opcode == SL_WEBSOCKET_OPCODE_CLOSE) {
        SlWebSocketFrameWriteOptions options = {0};
        if (server->config.websocket_frame != NULL) {
            SlStatus callback_status =
                server->config.websocket_frame(connection, &connection->request_arena, frame,
                                               out_diag, server->config.websocket_frame_user);
            if (!sl_status_is_ok(callback_status)) {
    ...
    if (frame->opcode == SL_WEBSOCKET_OPCODE_PONG) {
        return sl_status_ok();
    }
    if (!frame->fin || frame->opcode == SL_WEBSOCKET_OPCODE_CONTINUATION) {
    ...
    connection->websocket_message_count += 1U;
    return sl_status_ok();
}

static SlStatus sl_http_transport_websocket_receive(SlHttpTransportConnection* connection,
                                                    SlBytes bytes, SlDiag* out_diag)
{
    SlHttpTransportServer* server = sl_http_transport_connection_server(connection);
    SlStatus status;

    if (connection == NULL || server == NULL || (bytes.ptr == NULL && bytes.length != 0U)) {
        return sl_status_from_code(SL_STATUS_INVALID_ARGUMENT);
    }
    if (connection->state == SL_HTTP_TRANSPORT_CONNECTION_STATE_WRITING_RESPONSE) {
        return sl_http_transport_connection_diag(
            connection, out_diag, SL_DIAG_HTTP_RESPONSE_BACKPRESSURE, SL_STATUS_INVALID_STATE,
            sl_http_transport_literal("WebSocket frame arrived while a write is pending",
                                      sizeof("WebSocket frame arrived while a write is pending") -
                                          1U),
            sl_http_transport_literal("the native transport serializes WebSocket writes",
                                      sizeof("the native transport serializes WebSocket writes") -
                                          1U));
    }
    ...
    while (connection->accumulation_length != 0U) {
        SlWebSocketFrameParseOptions options = {0};
        SlWebSocketFrameParseResult parsed = {0};
        SlBytes accumulated =
            sl_bytes_from_parts(connection->accumulation, connection->accumulation_length);
        uint16_t close_code = 1002U;
        size_t remaining = 0U;

        sl_arena_reset(&connection->request_arena);
        options.require_mask = true;
        options.max_payload_bytes = server->config.parse.max_body_length;
        status = sl_websocket_parse_frame(&connection->request_arena, accumulated, &options,
                                          &parsed, out_diag);
        if (sl_status_code(status) == SL_STATUS_OUT_OF_RANGE) {
            return sl_status_ok();
        }
        if (!sl_status_is_ok(status)) {
            if (parsed.close_code != 0U) {
                close_code = parsed.close_code;
            }
            return sl_http_transport_websocket_send_close(connection, close_code, out_diag);
        }
        if (parsed.consumed == 0U || parsed.consumed > connection->accumulation_length) {
            return sl_http_transport_connection_diag(
                connection, out_diag, SL_DIAG_INVALID_HTTP_REQUEST, SL_STATUS_INTERNAL,
                sl_http_transport_literal("WebSocket frame parser made no progress",
                                          sizeof("WebSocket frame parser made no progress") - 1U),
                sl_http_transport_literal("frame parsing must consume a complete frame",
                                          sizeof("frame parsing must consume a complete frame") -
                                              1U));
        }
        ...
        sl_byte_builder_init_fixed(&connection->accumulation_builder, connection->accumulation,
                                   connection->accumulation_capacity);
        status = sl_byte_builder_append_bytes(
            &connection->accumulation_builder,
            sl_bytes_from_parts(connection->accumulation, connection->accumulation_length));
        if (!sl_status_is_ok(status)) {
            return status;
        }
        status = sl_http_transport_websocket_dispatch_frame(connection, &parsed.frame, out_diag);
        if (!sl_status_is_ok(status) ||
            connection->state == SL_HTTP_TRANSPORT_CONNECTION_STATE_WRITING_RESPONSE)
        {
            return status;
                                                ...
                                                SlBytes bytes, SlDiag* out_diag)
{
    ...
    if (connection->websocket_upgraded) {
        return sl_http_transport_websocket_receive(connection, bytes, out_diag);
    }
    if (connection->state != SL_HTTP_TRANSPORT_CONNECTION_STATE_ACCEPTED &&
        connection->state != SL_HTTP_TRANSPORT_CONNECTION_STATE_KEEP_ALIVE_IDLE &&
        connection->state != SL_HTTP_TRANSPORT_CONNECTION_STATE_READING_HEAD &&
    ...
    sl_http_profile_count(SL_HTTP_PROFILE_COUNTER_PARSER_BYTES, bytes.length);
    {
        uint64_t started_ns = sl_http_profile_now_ns();
        status = sl_http_transport_append_bytes(connection, bytes, out_diag);
        sl_http_profile_record_phase(SL_HTTP_PROFILE_PHASE_SOCKET_READ,
                                     sl_http_profile_now_ns() - started_ns);
    }
    if (!sl_status_is_ok(status)) {
        connection->state = SL_HTTP_TRANSPORT_CONNECTION_STATE_ERROR;
...
SlStatus sl_http_transport_server_stop(SlHttpTransportServer* server, SlDiag* out_diag)
{
    ...
    if (server->platform != NULL && server->platform->loop_initialized) {
        uv_run(&server->platform->loop, UV_RUN_NOWAIT);
    }
    ...
    for (size_t index = 0U; index < server->connection_capacity; index += 1U) {
        ...
        sl_http_transport_connection_close(&server->connections[index], NULL);
    }
