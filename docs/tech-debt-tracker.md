...
- Compiler/runtime completion for realistic supported Sloppy apps beyond ENGINE-02 and
  ...
  source locations. COMPILER-30.E covers supported provider registration/lookup metadata for
  the current database family, config reads, schema declarations, request bindings, and
  preliminary `Results.*` response metadata. COMPILER-30.F/G covers provider-kind-aware
  database effect/capability inference for direct provider calls and same-file helpers.
  COMPILER-30.H/I covers validation/completeness and strong Plan metadata. COMPILER-30.J
  adds the broad fixture/golden suite for realistic supported apps, partial completeness,
  multi-provider database metadata, and invalid provider/effect shapes. Cache reuse,
  broader async source shapes,
...
- Runtime feature modularity after ENGINE-27: the core registry, Plan-driven activation,
  current stdlib/provider/transport descriptor mapping, Plan-gated V8 intrinsic
...
- Time API implementation after CORE-TIME-01.C/D/E/F/G/H/I: `stdlib.time`, the `sloppy/time`
...
- Crypto API implementation after CORE-CRYPTO-01.I: OS CSPRNG random helpers,
  ...
  security-looking use. CORE-CRYPTO-01.I adds source examples, static example checks, and
  the consolidated conformance/docs/golden evidence index across the completed crypto
  surface.
...
- Network API implementation after CORE-NET-01.I: `stdlib.net`, the `sloppy/net`
  ...
  evidence index. CORE-HTTPCLIENT-01.D adds a separate cleartext HTTP/1.1 loopback client
  lane over the TCP bridge. Broader stress/torture evidence remains future work. Do not
  claim external network access, TLS, pooled/redirecting HTTP client behavior, UDP,
  WebSocket, Node/Bun/Deno compatibility, performance, or public alpha readiness from the
  current TCP slice.
