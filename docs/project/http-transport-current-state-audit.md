...
ENGINE-24.A/B is now implemented as a listener foundation. Sloppy has a Slop-owned HTTP
transport server boundary that can initialize, bind, listen, accept TCP connections, enforce
bounded connection capacity, stop, and dispose without exposing libuv handles through public
runtime or JavaScript APIs.
...
- Accepted connections claim bounded connection slots and start in `ACCEPTED` state.
...
- Stop/dispose close listener and accepted placeholder connection handles exactly once.

## Still Deferred

- #414 owns connection read loop and request accumulation.
