...
| #608 | V8/stdlib integration and JS surface | Implemented by the stdlib surface over the existing CORE-NET `__sloppy.net` V8 bridge, feature activation for `stdlib.httpclient`, and V8-gated bridge dependency evidence. A separate HTTP-native intrinsic namespace is not required for the HTTP/1.1-first public surface and can be future work if transport evolution needs it. |
