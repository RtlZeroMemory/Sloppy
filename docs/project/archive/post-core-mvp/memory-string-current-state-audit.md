...
Status: current-state audit for ENGINE-21 and ENGINE-22.
...
Slop now has the ENGINE-21 primitive layer for the non-interop foundation: borrowed `SlStr`
and `SlBytes`, arena-owned string/byte copy helpers, deterministic hash/equality helpers,
checked size math, caller-backed `SlArena`, bounded byte/string builders, bounded
app/static string interning, arena rollback patterns, deterministic diagnostics, scoped
cleanup, generation-counted resource IDs, and arena-owned parser/provider output in
several subsystems.

The remaining memory/string gaps are deliberate follow-ups: centralized V8/native
conversion policy, centralized SQLite text/blob ownership policy, a future standalone
heap/operation-owned `SlBuf`, stronger raw-allocation enforcement, fuzz/sanitizer hooks,
and ENGINE-22 adoption across hot paths.

The old open task `TASK 03.B: String Builder / Buffer Foundation` is now absorbed by
ENGINE-21.C for the primitive builder surface. ENGINE-22 tracks adoption/refactor work
across subsystems.
...
| --- | --- | --- | --- |
| Borrowed string view | `include/sloppy/string.h`, `src/core/string.c`, `tests/unit/core/test_string.c` | `SlStr` is pointer plus length, not owning and not necessarily NUL-terminated. Helpers now cover suffix checks, deterministic hashing, arena-owned copies, and explicit NUL boundary copies. | Safe/acceptable foundation. |
| Borrowed byte view | `include/sloppy/bytes.h`, `src/core/bytes.c`, `tests/unit/core/test_bytes.c` | `SlBytes` is pointer plus length for non-owning bytes. Helpers now cover prefix/suffix checks, deterministic hashing, and arena-owned binary copies. | Safe/acceptable foundation. |
...
| Byte/string builders | `include/sloppy/builder.h`, `src/core/builder.c`, `tests/unit/core/test_builder.c` | Fixed and arena-backed bounded builders support append/reserve, decimal formatting, optional NUL view, overflow/capacity checks, and prefix preservation after failure. | Safe/acceptable primitive foundation; subsystem adoption remains ENGINE-22. |
| String interning/symbol tables | `include/sloppy/intern.h`, `src/core/intern.c`, `tests/unit/core/test_intern.c` | Bounded app/static-lifetime table copies metadata strings into caller arena, stores hash/length, resolves bucket collisions with byte equality, and returns generation-tagged symbols. | Safe/acceptable primitive foundation; route/Plan/provider adoption remains ENGINE-22.C/F. |
...
| Diagnostics | Diagnostic renderers use length calculators plus custom writers. Capability/route/HTTP helpers build hints with local arena buffers. | Duplicated primitive, future optimization target. | ENGINE-22.B should consume the shared builder while preserving diagnostics goldens. |
...
| Plan/route/module metadata | Strings are copied into arenas and compared by byte equality; the bounded intern table exists but is not adopted into graphs yet. | Safe/acceptable now, future optimization target. | Repeated app-lifetime metadata should adopt the bounded intern table where it improves lookup and ownership clarity. |
...
| --- | --- | --- | --- | --- |
| V8/SQLite interop policy remains a follow-up. | Issue #367, `src/engine/v8/*`, `src/data/sqlite.c` | Core string/byte primitives exist, but central conversion helpers and SQLite blob/text interop policy are not implemented in this PR. | Future async/provider adoption could diverge without the policy task. | ENGINE-21.D follow-up, then ENGINE-22.D/E adoption. |
...
- ENGINE-21.A/B/C/F now provide the primitive contracts and focused unit/stress evidence
  for views, arena-owned copies, builders, formatting helpers, and bounded interning.
- ENGINE-21.D should remain the focused V8/SQLite interop policy follow-up.
- ENGINE-22 should migrate hot paths and remove duplicated ad hoc builders after the
  primitive layer lands.
