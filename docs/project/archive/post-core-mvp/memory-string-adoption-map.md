...
with ENGINE-22.A HTTP adoption, ENGINE-22.C Plan/artifact adoption, ENGINE-22.D V8
bridge adoption, ENGINE-22.E SQLite result/parameter adoption, and the non-SQLite
ENGINE-22.F cleanup/regression guard implemented for their bounded subsystem passes.
...
| V8 bridge string conversions `hot` | `src/engine/v8/engine_v8.cc`, `src/engine/v8/http_bridge.cc`, `src/engine/v8/intrinsics_sqlite.cc`, `src/engine/v8/string_interop.*` | ENGINE-22.D now routes engine-level native-to-V8 strings, request-context materialization, HTTP `Results.*` descriptor bodies/content types, JSON response bytes, and bridge exception strings through the private V8 string interop helpers. ENGINE-22.E extends that policy to SQLite bridge string parameters and row materialization. Result and diagnostic strings that escape the V8 call are arena-owned native views; request path/query/header/body values become V8-owned strings and do not retain native views. | Runtime source-map remapping and richer exception spans remain ENGINE-15; future provider/offload work must preserve the same operation-owned parameter rule. | Central V8 string conversion helpers and arena/request-owned native strings everywhere. | Predictable native/V8 lifetime, fewer duplicated helpers, safer async evolution. | High: V8-gated behavior and owner-thread policy. | ENGINE-22.D implemented for provider-neutral V8/HTTP bridge paths; ENGINE-22.E implemented for SQLite bridge result/parameter continuation. | ENGINE-14 module/bootstrap, ENGINE-15 diagnostics, ENGINE-17 SQLite bridge. |
| SQLite row/result conversion `hot` | `src/data/sqlite.c`, `include/sloppy/data_sqlite.h`, `src/engine/v8/intrinsics_sqlite.cc` | ENGINE-22.E keeps SQLite result column names, text cells, and blob cells arena-owned; routes V8 string parameters through the private V8 conversion helper and SQLite operation-owned parameter helper before provider calls; binds synchronous native text/blob parameters with SQLite transient ownership; and materializes JS rows with V8-owned strings or copied `Uint8Array` blobs. | Prepared statement policy, real async/offload submission, request-scope automatic provider cleanup, and broader row-shape/schema decisions remain future SQLite/runtime work. | SQLite text/blob ownership policy, owned parameter buffers, V8 conversion helpers. | Safe provider offload path, clear result lifetime, stable row mapping. | High: provider and V8 bridge interaction. | ENGINE-22.E implemented | ENGINE-17 SQLite runtime completion and ENGINE-19 SQLite conformance. |
| Diagnostics/source frames/JSON `hot on failures` | `src/core/diagnostics.c`, `src/core/capability.c`, `src/core/http.c`, `src/core/route.c`, `src/core/plan_parse.c` | ENGINE-22.B adopted shared bounded string builders for diagnostic text, JSON diagnostics, and source frames. ENGINE-22.F removes the remaining capability-denial hint pair buffer by using a bounded `SlStringBuilder` and a single arena-owned diagnostic hint. | Future source-map diagnostics can still expand subsystem-local formatting if new paths bypass the renderer. | String builder, formatter, redaction builder. | Stable output with less repeated formatting code and allocation-aware denial diagnostics. | Medium/high: golden snapshot drift risk. | ENGINE-22.B and ENGINE-22.F non-SQLite cleanup implemented | ENGINE-15 diagnostic completion. |
...
| CLI output | `src/main.c` | `SlCliSpan`, direct `printf`/`fwrite`, local JSON escaping, shared diagnostic redaction for doctor messages, diagnostic text rendering for `sloppy run` startup diagnostics, and builder-backed OpenAPI path skeleton normalization. | Some command output still streams directly and CLI-wide diagnostic-format selection remains deferred. OpenAPI remains a metadata skeleton, not real schema generation. | Shared string builder/diagnostic JSON builder and redaction formatter. | Consistent text/JSON output and fewer CLI-only fixed-buffer writers. | Medium: process golden output may change. | ENGINE-22.B and ENGINE-22.F non-SQLite cleanup implemented | ENGINE-18 CLI/dev loop and ENGINE-15 diagnostics. |
...
| Conformance/bench smoke | `tests/conformance/*`, `benchmarks/*`, `docs/testing*.md` | Conformance reports default vs V8-gated; benchmarks are harness/list/smoke and selected microbenchmarks. ENGINE-22.F adds a low-capacity capability-denial regression guard without changing benchmark labels or making performance claims. | Allocation behavior is not systematically guarded across every subsystem; benchmark smoke must not become performance claim. | Allocation-aware tests where possible, sanitizer/fuzz hooks, benchmark smoke labeling. | Regression visibility without marketing claims. | Medium: avoid noisy gates. | ENGINE-21.E and ENGINE-22.F non-SQLite guard implemented | ENGINE-19 conformance compatibility. |
...
5. ENGINE-22.D migrates V8 bridge conversions after #367 policy is accepted. Done for
   provider-neutral V8/HTTP bridge paths.
6. ENGINE-22.E migrates SQLite result/parameter conversion after #367 policy is accepted.
   Done for the current native SQLite provider and V8 SQLite bridge.
7. ENGINE-22.F removes duplicate ad hoc buffers/builders, finishes safe symbol adoption,
   and adds regression guards. The non-SQLite cleanup/guard pass is implemented.
