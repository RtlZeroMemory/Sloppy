...
`examples/sqlite-basic/app.js` shows SQLite service registration through
`data.sqlite.open({ database: ":memory:", capability: "data.main" })`. In a V8-enabled
runtime that installs the SQLite bridge and passes Plan/capability metadata into the
engine, that factory returns a safe wrapper around a resource-table connection handle.
