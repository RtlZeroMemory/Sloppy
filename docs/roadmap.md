...
| Time/deadline API | Partial | CORE-TIME-01.A/B defines `sloppy/time`, `stdlib.time`, compiler Plan activation, stable diagnostics, and bootstrap error classes. | Native timer backend, delay/timeout/cancellation implementation, intervals, scheduled jobs, fake clock, V8 bridge, existing-API integration, examples, and conformance remain in #554-#560. |
