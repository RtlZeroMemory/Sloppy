...
| ORM | supported app-host API | import recognized; static extraction partial | `features.orm`, `strongPlan.evidence.orm`, and dynamic ORM metadata emitted | supported when provider/runtime bridge is available | `sloppy/orm` provides table/column DSL, schemas, CRUD, query builder, includes, transactions, migration snapshot/diff helpers, raw SQL, cursors, and Sloppy-native ORM errors. Runtime use works dynamically; Plan table/relation extraction is intentionally marked partial until statically provable. |
