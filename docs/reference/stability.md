...
| Schema validation | supported | supported for static `Schema.*` declarations, `ctx.body.validate(...)`, `.accepts(...)`, and `.returns(...)` identifiers | supported Plan/OpenAPI metadata | supported for request/body validation through `ctx.body.validate(...)` and `.accepts(...)`; `.returns(...)` is metadata-only and runtime response enforcement is planned separately | Runtime schemas cover strings, numbers, integers, booleans, arrays, objects, enums, literals, optional, nullable, defaults, and common string/number bounds. Validation problems include path/code/message entries without raw body values. |
| Results helpers | supported | supported subset | supported | supported | `text`, `json`, `ok`, `created`, `accepted`, `noContent`, `notFound`, `badRequest`, `unauthorized`, `problem`, `status`, `html`, `bytes`, and bounded `stream` have compiler/runtime coverage where fixtures exercise them. `Results.stream` is still a bounded descriptor builder, not a WHATWG or Node stream. JSON result bodies use the alpha Sloppy serializer for dates, BigInts, bytes, undefined fields, class instances, errors, and circular-reference failures. |
| Native Core streams | n/a | n/a | n/a | experimental native foundation | `SlReadableStream`, `SlWritableStream`, bounded chunk-list/memory adapters, HTTP response stream adapter, pump, stats, cancellation, and backpressure statuses exist for native runtime components. No public JavaScript stream handle is exposed. |
...
| Realtime routes | experimental SSE API shape; WebSocket unavailable response; in-process hubs | supported for literal `app.sse(...)` and `app.ws(...)` route registrations | route `kind`, `runtime.realtime`, routes CLI, and OpenAPI extensions supported | SSE uses bounded `Results.stream`; WebSocket upgrade execution unavailable and returns `501` | SSE handlers build a bounded descriptor before returning; hubs are in-process bootstrap helpers, not external brokers or native WebSocket bindings. |
...
| Auth | supported | supported static subset | supported | supported with V8 stdlib crypto/codec/os features | JWT bearer is HS256-only with stricter `exp`/`nbf`/`iat` validation and optional clock skew; API keys use header validation; signed cookie sessions support sign-in/sign-out; route/group `requireAuth`, roles, claims, policies, Plan metadata, and OpenAPI security schemes are covered. OIDC discovery, JWKS, asymmetric JWT algorithms, OAuth flows, refresh tokens, and user management are not included. |
