...
| Realtime routes | experimental SSE API shape; WebSocket unavailable response; in-process hubs | supported for literal `app.sse(...)` and `app.ws(...)` route registrations | route `kind`, `runtime.realtime`, routes CLI, and OpenAPI extensions supported | SSE uses bounded `Results.stream` through native Core stream serialization; WebSocket upgrade execution unavailable and returns `501` | SSE handlers build a bounded descriptor before returning; hubs are in-process bootstrap helpers, not external brokers or native WebSocket bindings. |
...
| Jobs stdlib | supported app-host with provider-backed storage | supported from `sloppy/jobs` | `stdlib.jobs` feature emitted with scheduler metadata | JS scheduler runtime over SQLite/PostgreSQL/SQL Server data providers | Durable jobs, delayed jobs, recurring jobs, retries, dead-letter handling, idempotency, locks, worker leases, admin backend, and retention cleanup are covered by bootstrap tests. |
