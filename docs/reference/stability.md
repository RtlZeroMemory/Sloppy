...
| Schema validation | supported | supported for static `Schema.*` declarations, `ctx.body.validate(...)`, `.accepts(...)`, and `.returns(...)` identifiers | supported Plan/OpenAPI metadata | supported through Plan body validation where the route binding has a schema | Runtime enforcement applies to request/body validation through `ctx.body.validate(...)` and `.accepts(...)`; `.returns(...)` is compiler-visible response metadata for Plan/OpenAPI, not runtime response enforcement. |
