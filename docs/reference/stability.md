...
| TestServices | supported in JS tests | rejected import | n/a | Docker and provider bridge required | Starts real PostgreSQL/SQL Server containers through Docker CLI and verifies readiness through the matching data provider bridge. Default CI keeps live containers opt-in. |
