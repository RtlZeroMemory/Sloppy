...
| HttpClient / Http factory | supported API | low-level `HttpClient` supported from `sloppy/net`; factory metadata extraction is app-host only | low-level feature metadata is Plan-visible | experimental bridge | `Http` from `sloppy/http` provides named clients, typed clients, resilience policies, service registration, TestHost mocks, metrics, diagnostics, and health snapshots over the low-level pooled `HttpClient`. HTTP/1.1 by default; explicit h2/h2c, pooled h2 multiplexing, and HTTPS `auto` ALPN h2 selection are supported. `https://` needs the private outbound TLS bridge. |
