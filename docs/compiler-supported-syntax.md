...
| `builder.capabilities.addDatabase("token", { provider, access })` | Supported metadata extraction | Emits one Plan `dataProviders` entry and one database `capabilities` entry for first-party database providers (`sqlite`, `postgres`, `sqlserver`). No native provider is opened by the compiler. | Unsupported token/provider/access/shape diagnostics as applicable. | `provider-capability` golden fixture and COMPILER-30.F/G provider-kind unit coverage. | ENGINE-02 metadata; ENGINE-05/06 runtime enforcement. |
...
| Direct database provider calls through a static provider handle | Supported effect inference | The effect model records `capabilityKind`, `providerKind`, provider token, operation, and access. For the database adapter family, `query(...)` and `queryOne(...)` infer `read`; `exec(...)` infers `write` unless the SQL literal is read-only; `transaction(...)` infers `readwrite`. SQLite provider-backed route handlers are wrapped with request-scoped runtime handles in generated `app.js`; PostgreSQL and SQL Server metadata can be inferred before their JS bridges are executable. | Dynamic provider handles or unsupported provider methods are not inferred. | `effects-capability` golden fixture and COMPILER-30.F/G unit coverage. | COMPILER-30.F/G effect and capability inference. |
| Same-file helper functions that close over static provider handles | Supported effect inference | Named function helpers and `const helper = (...) => ...` initializers declared after the provider handle may contribute provider effects to routes that call them directly. The compiler emits helper source and embeds helpers inside provider-backed handler wrappers so closure-captured provider handles remain request-scoped where a runtime opener exists. | Recursive helpers, imported helper effects, broad repository/object/class patterns, non-database provider adapters, and dynamic calls remain deferred unless covered by later fixtures. | `effects-capability` golden fixture and helper inference unit coverage. | COMPILER-30.F/G first supported callgraph slice. |
