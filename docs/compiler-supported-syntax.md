...
| Optional `Random`, `Hash`, `Hmac`, `Password`, `ConstantTime`, `Secret`, or `NonCryptoHash` imports from `"sloppy/crypto"` | Supported for feature metadata and API contract binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.crypto"]`, `features.crypto`, and `strongPlan.evidence.crypto`. The runtime descriptor is known but unavailable by default until CORE-CRYPTO implementation PRs add vetted backends and V8 intrinsics. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_crypto_import_emits_plan_required_feature` and alias rejection unit coverage. | CORE-CRYPTO-01 crypto API. |
