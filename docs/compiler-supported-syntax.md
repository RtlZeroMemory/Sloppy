...
| Optional `TcpClient`, `TcpListener`, `TcpConnection`, or `NetworkAddress` imports from `"sloppy/net"` | Supported for feature metadata and API contract binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.net"]`, `features.network`, and `strongPlan.evidence.network`. The runtime descriptor is known but unavailable by default until CORE-NET implementation PRs add TCP backends and V8 intrinsics. Static connect/listen target metadata is a CORE-NET follow-up where the compiler can see literal host/port option objects; dynamic targets must remain partial/dynamic metadata. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_net_import_emits_plan_required_feature` and alias rejection unit coverage. | CORE-NET-01 TCP networking API. |
