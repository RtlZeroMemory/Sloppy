...
| Optional `Random`, `Hash`, `Hmac`, `Password`, `ConstantTime`, `Secret`, or `NonCryptoHash` imports from `"sloppy/crypto"` | Supported for feature metadata and current stdlib binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.crypto"]`, `features.crypto`, and `strongPlan.evidence.crypto`. CORE-CRYPTO-01.C/D/F/H implements Random, Hash, Hmac, ConstantTime, Secret, and the V8/std-lib surface; Password and NonCryptoHash fail closed until their later slices land. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_crypto_import_emits_plan_required_feature`, alias rejection unit coverage, core crypto vectors, bootstrap crypto coverage, and V8 crypto smoke coverage. | CORE-CRYPTO-01 crypto API. |
