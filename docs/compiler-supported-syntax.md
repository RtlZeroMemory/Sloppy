...
| Optional `File`, `Directory`, `Path`, `FileHandle`, or `FileWatcher` imports from `"sloppy/fs"` | Supported for feature metadata and current stdlib binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.fs"]` and binds the current stdlib FS namespace. `FileHandle` and `FileWatcher` remain deferred placeholder APIs until later CORE-FS slices. | Aliases or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `sloppy_fs_import_emits_plan_required_feature` unit coverage. | CORE-FS-01.C/D/H core FS bridge. |
