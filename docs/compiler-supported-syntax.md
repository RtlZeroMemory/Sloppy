...
| Strong Plan completeness metadata | Supported metadata emission | Emits route and whole-plan `completeness` plus `strongPlan` evidence. `complete` means required route/provider/body/response metadata is Plan-visible for the supported subset. `partial` is used for runnable routes with incomplete optional metadata, such as `Results.status(ctx.route.code, ...)` or `ctx.body.json()` without a schema. | Missing provider registration for an inferred effect is invalid and produces `SLOPPYC_E_MISSING_PROVIDER` before artifact emission. | Compiler unit coverage and refreshed success fixture goldens. | COMPILER-30.H/I completeness and strong Plan emission. |
