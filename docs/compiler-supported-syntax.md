...
- optional unaliased `Random`, `Hash`, `Hmac`, `Password`, `ConstantTime`, `Secret`, and
  `NonCryptoHash` imports from `"sloppy/crypto"` for Plan-visible crypto feature
  activation;
- optional unaliased `TcpClient`, `TcpListener`, `TcpConnection`, and `NetworkAddress`
...
| Optional `sql` import from `"sloppy/data"` | Supported for Framework v2 metadata fixtures | Accepted only as unaliased `sql` so realistic typed provider examples can parse. It does not add provider runtime behavior by itself. | Aliases or unknown imports use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `framework-v2-metadata` golden fixture. | FRAMEWORK-V2-COMPILER-01 metadata foundation. |
| Optional `Postgres` import from `"sloppy/providers/postgres"` | Supported for Framework v2 provider injection | Accepted only as unaliased `Postgres` for typed handler parameters such as `Postgres<"main">`. The typed parameter infers a `postgres/main` provider requirement, `data.main` database capability, and normal provider config requirement. | Aliases or unknown imports use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `framework-v2-metadata` golden fixture and provider-injection unit coverage. | FRAMEWORK-V2-COMPILER-01 provider metadata foundation. |
| Optional `SqlServer` import from `"sloppy/providers/sqlserver"` | Supported for Framework v2 provider injection | Accepted only as unaliased `SqlServer` for typed handler parameters such as `SqlServer<"main">`. The typed parameter infers a `sqlserver/main` provider requirement, `data.main` database capability, and normal provider config requirement. | Aliases or unknown imports use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `framework-v2-metadata` golden fixture and provider-injection unit coverage. | FRAMEWORK-V2-COMPILER-01 provider metadata foundation. |
| Optional `Sqlite` import from `"sloppy/providers/sqlite"` | Supported for Framework v2 provider injection | Accepted only as unaliased `Sqlite` for typed handler parameters such as `Sqlite<"main">`. The typed parameter infers a `sqlite/main` provider requirement, `data.main` database capability, and normal provider database config requirement. This is distinct from the lowercase `sqlite` runtime provider factory. | Aliases or unknown imports use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `framework-v2-metadata` golden fixture and provider-injection unit coverage. | FRAMEWORK-V2-COMPILER-01 provider metadata foundation. |
| Optional worker imports from `"sloppy/workers"` | Supported for worker feature metadata and Framework v2 queue injection | Accepted only as unaliased `BackgroundService`, `WorkQueue`, `WorkerPool`, `Worker`, `WorkerCancellationController`, `WorkerCancellationSignal`, or `SloppyWorkerError`. `WorkQueue<"name">` typed handler parameters infer a `queue.<name>` Plan capability and a default generated `WorkQueue.create("<name>")` service registration unless source code explicitly registers that token. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `framework-v2-metadata`, `typed_framework_queue_injection_infers_capability_and_default_service`, `sloppy_workers_import_emits_plan_required_feature`, and workers API-shape coverage. | FRAMEWORK-V2-COMPILER-01 queue metadata and CORE-WORKERS worker API. |
...
| Optional `Random`, `Hash`, `Hmac`, `Password`, `ConstantTime`, `Secret`, or `NonCryptoHash` imports from `"sloppy/crypto"` | Supported for feature metadata and current stdlib binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.crypto"]`, `features.crypto`, and `strongPlan.evidence.crypto`. CORE-CRYPTO-01.I implements Random, Hash, Hmac, Password, ConstantTime, Secret, NonCryptoHash, the V8/std-lib surface, and source example/conformance evidence. Static `NonCryptoHash.xxHash64` use in security-looking source emits a bounded doctor warning where visible. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_crypto_import_emits_plan_required_feature`, `sloppy_crypto_noncrypto_hash_security_context_emits_doctor_warning`, alias rejection unit coverage, core crypto vectors, password hashing/non-crypto vectors, bootstrap crypto coverage, `examples.crypto.api_shape`, and V8 crypto smoke coverage. | CORE-CRYPTO-01 crypto API. |
| Optional `TcpClient`, `TcpListener`, `TcpConnection`, or `NetworkAddress` imports from `"sloppy/net"` | Supported for feature metadata and API contract binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.net"]`, `features.network`, and `strongPlan.evidence.network`. CORE-NET-01.I implements the scoped runtime TCP client/listener surface plus local DNS/address/socket-option coverage, doctor/audit metadata goldens, source examples, and conformance indexing. Static connect/listen target metadata is a CORE-NET follow-up where the compiler can see literal host/port option objects; dynamic targets must remain partial/dynamic metadata. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_net_import_emits_plan_required_feature`, bootstrap `NetworkAddress` parsing coverage, native loopback TCP tests, `examples.net.api_shape`, network doctor/audit goldens, and alias rejection unit coverage. | CORE-NET-01 TCP networking API. |
| Optional `HttpClient` import from `"sloppy/net"` | Supported for feature metadata and current stdlib binding | Accepted only as an unaliased named import. The compiler emits `requiredFeatures: ["stdlib.httpclient"]`, `features.httpClient`, and `strongPlan.evidence.httpClient`. CORE-HTTPCLIENT-01 implements the cleartext HTTP/1.1 client lane over the CORE-NET TCP bridge, helper methods, bounded body consumption, timeout/deadline/cancellation behavior, redirects, pooling, DNS failure mapping, strict-network denial, redaction defaults, doctor/audit metadata goldens, and source example evidence. Compiler inference of literal static outbound targets and dynamic target metadata remains a follow-up; hand-authored Plan metadata can represent static or dynamic named clients for doctor/audit. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_net_http_client_import_emits_http_client_required_feature`, `bootstrap.stdlib.http_client`, `bootstrap.stdlib.app_host_foundation`, `examples.net.api_shape`, HTTP client doctor/audit goldens, and alias rejection unit coverage. | CORE-HTTPCLIENT-01 outbound HTTP client API. |
| Optional `System`, `Environment`, `Process`, or `Signals` imports from `"sloppy/os"` | Supported for feature metadata and current stdlib binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.os"]`, `features.os`, and `strongPlan.evidence.os`. CORE-OS-01 implements System/Environment, explicit-argv `Process.run`, `Process.start` handle facades, V8 process run/start scheduling, shutdown handler facade shape, doctor/audit metadata goldens, and source example evidence. Native platform signal loop integration remains deferred. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_os_import_emits_plan_required_feature`, `bootstrap.stdlib.os`, `examples.os.api_shape`, OS doctor/audit goldens, OS Plan parser fixtures, V8 OS process smoke coverage, and alias rejection unit coverage. | CORE-OS-01 host-system runtime API. |
| Optional `Base64`, `Base64Url`, `Hex`, `Text`, `Binary`, `Compression`, or `Checksums` imports from `"sloppy/codec"` | Supported for feature metadata and current API binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.codec"]`, `features.codec`, and `strongPlan.evidence.codec`. Base64/Base64Url/Hex/Text/Binary are stdlib helpers; Compression requires the active V8 `__sloppy.codec` zlib bridge; `Checksums.crc32` is a non-security stdlib helper. Static `Checksums.crc32` use in security-looking source emits a bounded doctor warning where visible. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_codec_import_emits_plan_required_feature`, `sloppy_codec_checksum_security_context_emits_doctor_warning`, bootstrap codec coverage, `examples.codec.api_shape`, and V8 codec smoke coverage. | CORE-CODEC-01 codec API. |
...
| Framework v2 typed handler parameters | Supported typed wrapper execution for the current compiler subset | Type-annotated handlers may use implicit body models, route parameter inference by name, explicit `Route<T>`, `Query<T>`, `Body<T>`, `Header<"name">`, `Service<T>`, context types such as `RequestContext`, provider markers `Postgres<"name">`, `Sqlite<"name">`, `SqlServer<"name">`, and `WorkQueue<"name">`. The compiler emits Plan metadata and a generated wrapper that awaits route/query/header/body/context/provider/queue/config materialization before handler invocation. The wrapper strips supported TypeScript-only parameter and call-generic syntax from the emitted handler source, creates a request service scope, and resolves services from literal `app.services.addSingleton/addScoped/addTransient("Token", factory)` registrations through the runtime Framework service provider. Typed database provider parameters infer default database capability/provider requirements from the parameter name and normal provider config; manual capability metadata is only needed for explicit policy overrides. SQLite provider injection opens the existing stdlib/native bridge by Plan token; PostgreSQL and SQL Server provider injection uses inferred database capability metadata plus the normal provider connection-string config key before opening the active bridge. `WorkQueue<"name">` infers a `queue.<name>` capability and default service registration, and `Config<"KEY">` reads an environment value. | Ambiguous primitives, multiple implicit bodies, missing route bindings, dynamic provider names, unknown generic markers, unresolved body types, and malformed `Header` wrappers produce source-aware diagnostics. Unsupported runtime injections fail explicitly instead of returning fake provider objects. | `framework-v2-metadata` golden fixture, native Plan parse/dispatch validation tests, `conformance.framework_v2_di_services.run_once`, `typed_framework_body_bindings_are_awaited_before_handler_entry`, `typed_framework_provider_injection_uses_configured_provider_options`, `typed_framework_queue_injection_infers_capability_and_default_service`, and `typed_framework_negative_diagnostics_are_source_aware` unit coverage. | FRAMEWORK-V2-COMPILER-01 plus #673 validation foundation, typed wrapper execution, DI/runtime-provider injection, and first config/queue injection slice. |
| Destructured/default/rest or untyped multiple handler parameters | Rejected with diagnostic | Rejected before artifact emission. Typed Framework v2 signatures are the supported multiple-parameter path. | `SLOPPYC_E_UNSUPPORTED_HANDLER_PARAMETERS`. | `unsupported-handler-parameter` diagnostic fixture. | Framework v2 metadata supports only typed simple identifiers. |
...
| TypeScript type aliases and interfaces for Framework v2 models | Supported focused subset | Emits schema metadata for concrete aliases/interfaces, object literals, primitive properties, nested objects, arrays, optional properties, nullable unions, literal unions, and semantic built-ins `Email`, `NonEmptyString`, `PasswordString<N>`, `SecretString`, `Uuid`, `PositiveInt`, `DateTime`, and `Instant`. Password/secret shapes are marked redacted in Plan metadata. | Generic aliases/interfaces, conditional/mapped/recursive/imported or otherwise unsupported shapes produce source-aware diagnostics rather than partial schemas. | `framework-v2-metadata` golden fixture and negative unit coverage. | FRAMEWORK-V2-COMPILER-01 schema metadata foundation. |
| `Results.text/json/bytes/ok/created/accepted/html/status/noContent/badRequest/notFound/problem` metadata | Supported preliminary metadata extraction | Accepted result helpers still govern handler support; routes with request/config/schema metadata include preliminary response kind/status/helper metadata in Plan. `Results.bytes` compiler support is intentionally narrow to inline `new Uint8Array([...])` byte literals plus JSON-safe options. | Unsupported helper shapes continue to produce handler diagnostics. | `metadata-extraction` and result helper unit coverage. | COMPILER-30.E results metadata; completeness in COMPILER-30.H/I. |
| Framework v2 `Results.ok/created/notFound/...` response metadata | Supported metadata extraction | Typed Framework v2 handlers may contain multi-statement bodies and conditional result returns. The compiler records deterministic status/helper/kind metadata and source locations for visible `Results.*` helpers. This is OpenAPI-ready metadata only, not an OpenAPI exporter or response-writer runtime. | Unsupported or dynamic result shapes remain diagnostics or incomplete metadata according to the existing result-helper contract. | `framework-v2-metadata` golden fixture. | FRAMEWORK-V2-COMPILER-01 response metadata foundation. |
...
| TypeScript input and Framework v2 typed handler syntax | Supported focused subset | `.ts` input is accepted for the documented Framework v2 source subset. The compiler erases supported parameter annotations, call generics, `as`, `satisfies`, and non-null assertion syntax in emitted handler wrappers. Unsupported TypeScript shapes and full type checking remain out of scope; `.tsx/.mts/.cts` remain rejected as unsupported source extensions. | `SLOPPYC_E_UNSUPPORTED_INPUT` for unsupported extensions; unsupported handler shapes or unsupported schemas produce source-aware diagnostics such as `SLOPPYC_E_UNSUPPORTED_TYPESCRIPT_HANDLER` where applicable. | `framework-v2-metadata`, `unsupported-typescript-handler`, parser module extension tests, and typed-handler erasure unit coverage. | Focused Framework v2 lowering only; official broad TypeScript checking/lowering later. |
