...
| Optional `Time`, `Deadline`, `CancellationController`, `TimeoutError`, `CancelledError`, `InvalidDeadlineError`, or `TimerDisposedError` imports from `"sloppy/time"` | Supported for feature metadata and current API contract binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.time"]`. Runtime timer execution is deferred to CORE-TIME-01.C/D/G. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `sloppy_time_import_emits_plan_required_feature` unit coverage. | CORE-TIME-01 Time API. |
