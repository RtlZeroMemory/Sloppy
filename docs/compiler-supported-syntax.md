...
| Optional `System`, `Environment`, `Process`, or `Signals` imports from `"sloppy/os"` | Supported for feature metadata and current stdlib binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.os"]`, `features.os`, and `strongPlan.evidence.os`. CORE-OS-01 implements System/Environment, explicit-argv `Process.run`, `Process.start` handle facades, shutdown handler facade shape, doctor/audit metadata goldens, and source example evidence. V8 process bridge scheduling and native platform signal loop integration remain deferred. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_os_import_emits_plan_required_feature`, `bootstrap.stdlib.os`, `examples.os.api_shape`, OS doctor/audit goldens, OS Plan parser fixtures, and alias rejection unit coverage. | CORE-OS-01 host-system runtime API. |
