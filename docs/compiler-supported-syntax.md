...
| Framework v2 typed handler parameters | Supported metadata extraction, runtime dispatch deferred | Type-annotated handlers may use implicit body models, route parameter inference by name, explicit `Route<T>`, `Query<T>`, `Body<T>`, `Header<"name">`, context types such as `RequestContext`, provider markers `Postgres<"name">`, `Sqlite<"name">`, `SqlServer<"name">`, and `WorkQueue<"name">`. The compiler emits Plan metadata and a generated handler that returns a 501 problem descriptor; it does not implement HTTP dispatch, DI, validation, provider execution, or TypeScript runtime reflection. | Ambiguous primitives, multiple implicit bodies, missing route bindings, dynamic provider names, unknown generic markers, unresolved body types, and malformed `Header` wrappers produce source-aware diagnostics. | `framework-v2-metadata` golden fixture and `typed_framework_negative_diagnostics_are_source_aware` unit coverage. | FRAMEWORK-V2-COMPILER-01 metadata foundation. |
