...
| Framework v2 typed handler parameters | Supported typed wrapper execution for the current compiler subset | Type-annotated handlers may use implicit body models, route parameter inference by name, explicit `Route<T>`, `Query<T>`, `Body<T>`, `Header<"name">`, context types such as `RequestContext`, provider markers `Postgres<"name">`, `Sqlite<"name">`, `SqlServer<"name">`, and `WorkQueue<"name">`. The compiler emits Plan metadata and a generated wrapper that materializes route/query/header/body/context arguments after native Plan-backed validation. The wrapper strips supported TypeScript-only parameter and call-generic syntax from the emitted handler source. Provider/queue/service injections fail with explicit unavailable diagnostics unless the current runtime lane can materialize that dependency. Full DI, custom validators, arbitrary TypeScript lowering, and TypeScript runtime reflection remain unsupported. | Ambiguous primitives, multiple implicit bodies, missing route bindings, dynamic provider names, unknown generic markers, unresolved body types, and malformed `Header` wrappers produce source-aware diagnostics. Unsupported runtime injections fail explicitly instead of returning fake provider objects. | `framework-v2-metadata` golden fixture, native Plan parse/dispatch validation tests, and `typed_framework_negative_diagnostics_are_source_aware` unit coverage. | FRAMEWORK-V2-COMPILER-01 plus #673 validation foundation and typed wrapper execution slice. |
