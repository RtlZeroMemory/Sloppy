...
| Framework v2 typed handler parameters | Supported typed wrapper execution for the current compiler subset | Type-annotated handlers may use implicit body models, route parameter inference by name, explicit `Route<T>`, `Query<T>`, `Body<T>`, `Header<"name">`, `Service<T>`, context types such as `RequestContext`, provider markers `Postgres<"name">`, `Sqlite<"name">`, `SqlServer<"name">`, and `WorkQueue<"name">`. The compiler emits Plan metadata and a generated wrapper that materializes route/query/header/body/context arguments after native Plan-backed validation. The wrapper strips supported TypeScript-only parameter and call-generic syntax from the emitted handler source, creates a request service scope, and resolves services from literal `app.services.addSingleton/addScoped/addTransient("Token", factory)` registrations. SQLite provider injection opens the existing stdlib/native bridge by Plan token; PostgreSQL and SQL Server provider injection attempts the provider bridge and reports the bridge/config/live-lane error honestly when unavailable. Queue and `Config<T>` injection still require explicit runtime support. | Ambiguous primitives, multiple implicit bodies, missing route bindings, dynamic provider names, unknown generic markers, unresolved body types, and malformed `Header` wrappers produce source-aware diagnostics. Unsupported runtime injections fail explicitly instead of returning fake provider objects. | `framework-v2-metadata` golden fixture, native Plan parse/dispatch validation tests, `conformance.framework_v2_di_services.run_once`, and `typed_framework_negative_diagnostics_are_source_aware` unit coverage. | FRAMEWORK-V2-COMPILER-01 plus #673 validation foundation, typed wrapper execution, and first DI runtime slice. |
