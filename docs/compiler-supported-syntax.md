...
| Optional `TcpClient`, `TcpListener`, `TcpConnection`, or `NetworkAddress` imports from `"sloppy/net"` | Supported for feature metadata and API contract binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.net"]`, `features.network`, and `strongPlan.evidence.network`. CORE-NET-01.G implements the scoped runtime TCP client/listener surface plus local DNS/address/socket-option coverage. Static connect/listen target metadata is a CORE-NET follow-up where the compiler can see literal host/port option objects; dynamic targets must remain partial/dynamic metadata. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_net_import_emits_plan_required_feature`, bootstrap `NetworkAddress` parsing coverage, native loopback TCP tests, and alias rejection unit coverage. | CORE-NET-01 TCP networking API. |
