...
| Optional `Postgres` import from `"sloppy/providers/postgres"` | Supported for Framework v2 provider injection metadata | Accepted only as unaliased `Postgres` for typed handler parameters such as `Postgres<"main">`. It does not add provider runtime behavior by itself. | Aliases or unknown imports use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `framework-v2-metadata` golden fixture. | FRAMEWORK-V2-COMPILER-01 provider metadata foundation. |
| Optional `SqlServer` import from `"sloppy/providers/sqlserver"` | Supported for Framework v2 provider injection metadata | Accepted only as unaliased `SqlServer` for typed handler parameters such as `SqlServer<"main">`. It does not add provider runtime behavior by itself. | Aliases or unknown imports use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `framework-v2-metadata` golden fixture. | FRAMEWORK-V2-COMPILER-01 provider metadata foundation. |
| Optional `Sqlite` import from `"sloppy/providers/sqlite"` | Supported for Framework v2 provider injection metadata | Accepted only as unaliased `Sqlite` for typed handler parameters such as `Sqlite<"main">`. This is distinct from the lowercase `sqlite` runtime provider factory. | Aliases or unknown imports use `SLOPPYC_E_UNSUPPORTED_IMPORT`. | `framework-v2-metadata` golden fixture. | FRAMEWORK-V2-COMPILER-01 provider metadata foundation. |
| Optional worker imports from `"sloppy/workers"` | Supported for worker feature metadata and Framework v2 queue injection metadata | Accepted only as unaliased `BackgroundService`, `WorkQueue`, `WorkerPool`, `Worker`, `WorkerCancellationController`, `WorkerCancellationSignal`, or `SloppyWorkerError`. `WorkQueue<"name">` typed handler parameters emit queue injection metadata only. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `framework-v2-metadata`, `sloppy_workers_import_emits_plan_required_feature`, and workers API-shape coverage. | FRAMEWORK-V2-COMPILER-01 queue metadata and CORE-WORKERS worker API. |
