...
| Framework v2 typed handler parameters | Supported metadata extraction, typed handler execution deferred | Type-annotated handlers may use implicit body models, route parameter inference by name, explicit `Route<T>`, `Query<T>`, `Body<T>`, `Header<"name">`, context types such as `RequestContext`, provider markers `Postgres<"name">`, `Sqlite<"name">`, `SqlServer<"name">`, and `WorkQueue<"name">`. The compiler emits Plan metadata and a generated handler that returns a 501 problem descriptor after the native Plan-backed validation boundary. Native dispatch can consume route/query/header scalar bindings and schema-backed JSON bodies; it does not implement typed handler execution, DI, provider execution, custom validators, or TypeScript runtime reflection. | Ambiguous primitives, multiple implicit bodies, missing route bindings, dynamic provider names, unknown generic markers, unresolved body types, and malformed `Header` wrappers produce source-aware diagnostics. | `framework-v2-metadata` golden fixture, native Plan parse/dispatch validation tests, and `typed_framework_negative_diagnostics_are_source_aware` unit coverage. | FRAMEWORK-V2-COMPILER-01 plus #673 validation foundation. |
