...
| Optional `Base64`, `Base64Url`, `Hex`, `Text`, `Binary`, `Compression`, or `Checksums` imports from `"sloppy/codec"` | Supported for feature metadata and API contract binding | Accepted only as unaliased named imports. The compiler emits `requiredFeatures: ["stdlib.codec"]`, `features.codec`, and `strongPlan.evidence.codec`. The runtime descriptor is known but unavailable by default until CORE-CODEC implementation PRs add the native/stdlib/V8 surface. | Aliases, side-effect imports, empty imports, or unknown names use `SLOPPYC_E_UNSUPPORTED_IMPORT` or `SLOPPYC_E_UNSUPPORTED_IMPORT_SPECIFIER`. | `sloppy_codec_import_emits_plan_required_feature` and alias rejection unit coverage. | CORE-CODEC-01 codec API. |
