...
| --- | --- | --- | --- | --- |
| SQLite | Native C provider exists and is linked through the default build. | In-memory open/query/exec/transaction and capability hook tests run in default CTest. | No external service is required for current provider tests. | Deferred until MAIN1-08; `data.sqlite.open(...)` fails with bridge-unavailable today. Capability hook integration is ready for that bridge. |
