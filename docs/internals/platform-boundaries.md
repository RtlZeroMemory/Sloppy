...
- `src/platform/*` owns OS-specific implementation code.
- `include/sloppy/platform.h` owns detection macros only.
- `tools/windows/check-platform-boundaries.ps1` and Unix equivalents enforce
  boundary rules.
- CMake presets select platform-specific build lanes.

## Main Concepts

The platform boundary keeps core runtime code portable while still allowing
Windows, POSIX, Linux, and macOS implementations behind Sloppy-owned APIs.
Platform support is a backend concern, not a reason to scatter `#ifdef` and OS
headers through core modules.

```mermaid
flowchart TB
    Core[Core runtime modules] --> API[Sloppy-owned platform API]
    API --> Common[src/platform/common]
    API --> Win32[src/platform/win32]
    API --> Posix[src/platform/posix]
    API --> Linux[src/platform/linux]
    API --> Mac[src/platform/macos]
    Win32 --> WinAPI[Windows APIs]
    Posix --> POSIX[POSIX APIs]
    Linux --> LinuxAPI[Linux APIs]
    Mac --> MacAPI[macOS APIs]
    Scanner[platform boundary scanner] -. forbids OS headers in core .-> Core
```

## Lifecycle

A platform-backed feature starts with a Sloppy-owned abstraction, adds backend
implementations under the matching platform directory, adds tests for both the
portable contract and backend-specific behavior, then updates scanners and docs
when the allowed boundary changes.

## Invariants

- OS headers are forbidden outside allowed platform implementation directories.
- Core modules call Sloppy-owned APIs instead of WinAPI/POSIX functions.
- Transport backends do not leak OS or libuv handles into public APIs.
- Platform-specific CI validates only the configured lane it ran.

## Failure Behavior

Unsupported platform behavior should fail with an explicit diagnostic or build
configuration error. Missing SDKs, tools, drivers, and platform dependencies are
environment blockers or unavailable lanes and should be reported that way.

## Public API Relationship

Public docs describe platform status and install/build support. Internally, this
page governs where OS behavior may be implemented and how platform status must
be validated before it appears in product docs.

## Tests And Evidence

Coverage comes from platform-boundary scanner self-tests, Windows/Linux/macOS
default CI lanes, C standards checks, CMake preset checks, and backend-specific
tests when a platform API is implemented.

## Current Limits

Windows x64 is the most complete local development lane. Linux and macOS are
product targets but do not carry the same V8/package/live-provider evidence by
default.
...
backend boundary. See `docs/internals/async-runtime.md`.
...
The self-test validates two fixture classes:
...
The scanner is intentionally lexical and conservative. It validates the
documented forbidden header boundary; it is not a C parser and may miss direct
OS symbol references that do not use forbidden includes.
...
- scanner tests validating forbidden include detection.
...
These presets are intentionally small Debug non-V8 gates. They validate the
portable core, compiler, default provider tests, and static boundaries on
hosted runners. V8 SDK validation, live database services, release package
smoke, sanitizers, and full platform feature parity use separate lanes. CMake
should not bake Windows-only assumptions into core targets. Platform selection
...
- scanner self-tests validate positive and allowed-boundary fixture behavior;
