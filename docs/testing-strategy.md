...
## TEST-PLATFORM-01 Governance

TEST-PLATFORM-01 (#654) is the repository test-governance baseline. Future PRs must
explain test intent, source-of-truth contracts, evidence lanes, skipped/unavailable lanes,
negative paths, golden updates, redaction checks, and deferred coverage. A passing test
suite is insufficient if the tests only snapshot current behavior.

Standard evidence lanes:

| Lane | Meaning |
| --- | --- |
| default non-V8 | Native default build and CTest lane. It must not depend on V8, live providers, package archives, long fuzzing, torture, or benchmark infrastructure. |
| compiler/Plan | Rust compiler, Plan parser/validator, fixture/golden, and source-map evidence. |
| V8-gated | Tests that require `SLOPPY_ENABLE_V8=ON` and the compatible V8 SDK. |
| source-input | `sloppy run <source>` or `sloppy.json` handoff fixtures. This proves compiler handoff and artifact emission, not default V8 execution unless paired with V8-gated evidence. |
| package outside-checkout | Extracted package/archive smoke tests that run outside the repository checkout and prove artifact layout. |
| platform-specific | Windows/Linux/macOS-specific behavior that must PASS, SKIP, or report UNAVAILABLE honestly per platform. |
| dependency-backed | Deterministic tests using local dependencies such as SQLite, zlib, libuv, llhttp, OpenSSL, sodium, or xxHash. |
| live-network/live-provider | External network, PostgreSQL, SQL Server, or service-backed tests. Never default. |
| fuzz/property | Deterministic property/seed replay by default when stable, and opt-in libFuzzer mutation runs for longer coverage. |
| stress/torture | Repeated, crash-resistance, race, and shutdown stress. Opt-in unless explicitly bounded and stable. |
| sanitizer/memory-safety | ASan/UBSan/TSan or equivalent memory-safety lanes. |
| benchmark | Harness smoke or measurements. Benchmarks are never correctness evidence. |

Status vocabulary is PASS, FAIL, SKIPPED, UNAVAILABLE, DEFERRED, and NOT RUN. Skipped
optional gates are not pass claims. Optional lanes must not be folded into default non-V8
evidence.

Golden policy under TEST-PLATFORM-01:

- structured JSON goldens assert semantic fields and normalized values;
- text goldens are for deliberate UX only;
- paths, source locations, line endings, timestamps, nondeterministic IDs, and
  platform-specific values must be normalized;
- real secrets must never appear;
- golden updates must explain the intended behavior change instead of approving current
  output blindly.

Source-input fixtures live under `tests/fixtures/source-input/` with `case.json`,
`sloppy.json`, `src/app.ts`, and semantic expected Plan/doctor/diagnostic files. Package
fixtures live under `tests/fixtures/package/` and describe extracted package expectations.
These fixtures are manually reviewed inputs, not generated by the compiler path they test.
`tools/windows/test-source-input-fixtures.ps1` executes the source-input cases and compares
their semantic Plan/doctor/diagnostic contracts instead of merely checking that files exist.

Fuzz/property coverage has two layers. The default-safe layer builds standalone seed replay
executables and runs the committed seed corpus through CTest labels `fuzz;property;seed-replay`.
The opt-in libFuzzer layer is enabled with `SLOPPY_ENABLE_LIBFUZZER=ON` and builds mutation
targets for Plan parsing, route patterns, HTTP request parsing, and diagnostic rendering.
Invalid fuzz input should either succeed or produce a stable diagnostic; it must not crash
unless a property assertion detects a contract violation such as failed redaction.

The current TEST-PLATFORM-01 inventory is:

| Area | Current owner |
| --- | --- |
| Unit/native | `tests/unit/core`, `tests/unit/data`, `tests/unit/engine` |
| Integration/source-input/runtime artifacts | `tests/integration`, `tests/cmake/check_source_input_run.cmake`, `tests/fixtures/source-input` |
| Conformance aliases and indexes | `tests/conformance`, including cross-API, codec, crypto, net, and V8 indexes |
| Golden surfaces | `tests/golden`, `compiler/tests/fixtures`, and CLI/doctor/Plan snapshots |
| Bootstrap stdlib | `tests/bootstrap` |
| Fuzz/property | `tests/fuzz` seed replay and opt-in libFuzzer targets |
| Package | `tools/windows/package.ps1`, `tools/windows/test-package.ps1`, and `tests/fixtures/package` |
| Static governance | `tools/windows/check-test-governance.ps1`, PR template, AGENTS, CONTRIBUTING, and `docs.test_platform_contract` |

Weak or stale tests should be fixed when low-risk. Larger cleanup belongs in an issue-backed
deferral rather than being hidden by updated goldens or broad skipped tests.
...
- ENGINE-19.E package outside-checkout smoke must extract the archive outside the checkout,
  ...
  stdlib assets, and run a prebuilt artifact fixture when metadata declares
  `mustNotCompileSource=true`. Building a tiny supported app with packaged `sloppyc` is a
  separate compiler-from-package smoke and must not be used as package artifact evidence. A
  default non-V8 package may prove layout and artifact-loading behavior while reporting
  artifact execution as skipped/not configured because V8 is unavailable; that is not V8
  execution, release readiness, package-manager
