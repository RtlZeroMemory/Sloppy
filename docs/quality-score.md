...
| Data providers | Yellow | Native SQLite, PostgreSQL, and SQL Server provider boundaries plus bootstrap data API/fake provider, core JS-safe resource IDs, capability-enforced V8-gated SQLite JS-to-native bridge, and ENGINE-12.CD per-provider-instance executor policy with bounded admission, copied inputs, cancellation/timeout/shutdown terminal states, and cleanup-once native tests. | SQLite native in-memory tests; SQLite JS wrapper mocked-bridge tests; V8-gated SQLite bridge/capability tests when SDK is available; PostgreSQL/SQL Server default non-live tests; redaction, option, tiny pool/resource lifecycle tests; `core.provider_executor` default native tests. | PostgreSQL and SQL Server live tests are separate opt-in CTest gates that skip when env vars are absent; PostgreSQL/SQL Server JS-to-native bridges, full ENGINE-23 provider execution/offload runtime, SQLite async/offload conversion, request-scope provider ownership, production pooling, and stress evidence are missing. | Implement ENGINE-23 provider execution runtime first, then ENGINE-17 SQLite runtime completion through serialized provider executors, optional live service infrastructure, production pool policy, and future provider bridges through `intrinsics_<provider>.cc`. |
