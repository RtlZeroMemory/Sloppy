...
| Network APIs | Red | CORE-NET-01.A/B defines the public `sloppy/net` TCP contract, `stdlib.net` feature descriptor, compiler Plan activation, development/strict network policy, lifecycle model, stable diagnostics, and redaction policy. | Native TCP backend, libuv connect/listen, TcpClient/TcpConnection/TcpListener, streams/backpressure/deadline/cancellation, DNS/address/socket options, V8/stdlib surface, examples, doctor/audit, and conformance evidence remain unimplemented. |
