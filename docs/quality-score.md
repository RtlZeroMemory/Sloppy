...
| Data providers | Yellow | Native SQLite, PostgreSQL, and SQL Server provider boundaries plus bootstrap data API/fake provider, core JS-safe resource IDs, capability-enforced V8-gated SQLite JS-to-native bridge, and ENGINE-23.A/B per-provider-instance descriptor/admission foundation with operation kinds, capability metadata, bounded admission, copied inputs, cancellation/timeout/shutdown terminal states, and cleanup-once native tests. | SQLite native in-memory tests; SQLite JS wrapper mocked-bridge tests; V8-gated SQLite bridge/capability tests when SDK is available; PostgreSQL/SQL Server default non-live tests; redaction, option, tiny pool/resource lifecycle tests; `core.provider_executor` default native tests. | PostgreSQL and SQL Server live tests are separate opt-in CTest gates that skip when env vars are absent; PostgreSQL/SQL Server JS-to-native bridges, serialized blocking worker execution, blocking pools, SQLite async/offload conversion, request-scope provider ownership, production pooling, and stress evidence are missing. | Continue ENGINE-23 with #393 serialized blocking executor, #394 blocking pool, #395 cancellation/late completion, #396 capability-gated dispatch, and #397 diagnostics/stress evidence, then route ENGINE-17 SQLite runtime completion through provider executors. |
