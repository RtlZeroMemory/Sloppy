...
| Diagnostics | Yellow | Stable codes, text/JSON/source-frame rendering, redaction helpers, provider/runtime diagnostics, compiler source maps with deterministic Sloppy metadata, and V8-gated exception primary-span remapping. | Async diagnostic JSON/source frames, richer CLI format plumbing, structured fixes. |
...
| V8 integration | Yellow | Optional V8 execution, handlers, bounded direct Promise settlement, SQLite bridge, owner-thread checks, and source-map-remapped exception primary spans. | SDK packaging/CI, ESM module loading, timers/fetch/native async sources, async stack/source-frame remapping. |
