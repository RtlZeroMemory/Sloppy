...
| Memory/string foundations | Yellow | Borrowed `SlStr`/`SlBytes`, arena-owned string/byte copies, deterministic hash/equality helpers, checked math, caller-backed `SlArena`, bounded `SlByteBuilder`/`SlStringBuilder`, bounded app/static `SlInternTable`, arena rollback patterns, diagnostic arena builders, private V8 string interop helpers, SQLite text/blob copy helpers, arena-owned Plan/HTTP/SQLite outputs, and JS-safe resource IDs. | Core C tests, builder/intern tests, diagnostics tests, Plan parser tests, HTTP parser/response tests, SQLite provider tests, C standards scanner warnings for raw allocation, and V8-gated string conversion coverage when SDK evidence is available. | No standalone heap/operation-owned `SlBuf`, no broad ENGINE-22 hot-path adoption pass, no allocator misuse hard failure, and sanitizer/fuzz hooks remain optional/future. | Complete ENGINE-22 adoption across HTTP, V8, SQLite, diagnostics, Plan/artifacts, CLI, and conformance. |
