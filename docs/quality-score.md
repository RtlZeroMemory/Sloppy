...
| V8 integration | Yellow | Optional V8 execution, handlers, bounded direct Promise settlement, SQLite bridge, owner-thread checks, async rejection JSON diagnostics, native async scheduler tests, and source-map-remapped exception primary spans. | SQLite bridge not executor-backed, SDK packaging/CI, ESM module loading, timers/fetch/native async sources, async stack remapping. |
