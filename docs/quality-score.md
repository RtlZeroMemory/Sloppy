...
Status: 2026-05-06 post-CORE-NET-02 stabilization.
...
| Time/deadline/cancellation | Green | CORE-TIME-01.A/B defines the public `sloppy/time` API contract, `stdlib.time` feature descriptor, compiler Plan activation, stable error class names, and diagnostic codes. CORE-TIME-01.C/D/G adds the V8-gated native delay backend, owner-thread Promise settlement, `Time.delay`, `Time.timeout`, `Deadline`, `CancellationController`, and inactive-feature coverage. CORE-TIME-01.E/F adds async iterable intervals, interval-based scheduled jobs, no-overlap skipped-run reporting, `Time.systemClock`, and explicit `Time.fakeClock` deterministic tests. CORE-TIME-01.H aligns filesystem facade options plus app/request/provider/HTTP naming boundaries. CORE-TIME-01.I adds source examples, final diagnostic goldens, and a conformance evidence index. | Broader shutdown/cancellation consumers remain future work outside CORE-TIME-01. |
...
| Security/capabilities | Yellow | Plan/provider/capability metadata, registry checks, provider-executor admission, SQLite bridge enforcement, Plan-driven `sloppy capabilities`/audit visibility for compiler-generated provider effects, CORE-FS-01 filesystem feature/capability policy, core filesystem path-policy enforcement, resource-table-backed FileHandle/FileWatcher IDs, filesystem doctor/audit goldens for readwrite/watch/delete metadata, CORE-FS-02 separation of trusted runtime artifact reads from app filesystem policy, CORE-NET-01.I network capability doctor/audit metadata visibility, and CORE-NET-02 local IPC named-root/path policy plus doctor/audit metadata visibility. | No OS sandbox; audit remains static metadata review for network and local IPC grants. |
| Network APIs | Green | CORE-NET-01.C/D/H adds the Slop-owned native TCP client contract, private libuv connect/read/write/close backend, `TcpClient`/`TcpConnection` stdlib surface, feature-gated `__sloppy.net` V8 bridge, JS-safe TCP resource IDs, owner-thread Promise settlement, and deterministic loopback client conformance including embedded-NUL bytes. CORE-NET-01.E/F adds native `TcpListener`, ephemeral loopback listen, accept, listener resource handles, JS async accept iteration, close/abort, and accept-timeout coverage. CORE-NET-01.G adds `NetworkAddress` object/text parsing, local hostname/DNS-backed listen coverage, IPv4/IPv6 address handling, and `noDelay`/`keepAlive` unsupported-option propagation. CORE-NET-01.I adds doctor/audit goldens, source examples, and a conformance evidence index. CORE-NET-02 adds `LocalEndpoint`, `UnixSocket`, and `NamedPipe`, POSIX Unix domain socket and Windows named pipe native backends, named-root endpoint validation, stale socket cleanup, permission-mode honesty, bounded binary read/write helpers, deadline/cancellation handling, V8 bridge resource IDs, doctor/audit goldens, examples, and conformance evidence. CORE-HTTPCLIENT-01.D adds a separate bootstrap loopback lane for cleartext HTTP/1.1 client request/response execution over the TCP bridge. | External live-network, stress/torture, package, benchmark, TLS, pooled/redirecting HTTP client behavior, UDP, WebSocket, and Node/Bun/Deno compatibility remain future/out of scope. |
| Crypto APIs | Green | CORE-CRYPTO-01.A/B/C/D/E/F/G/H/I implements the scoped `sloppy/crypto` API: OS random helpers, SHA-2/HMAC vectors, ConstantTime, Secret cleanup, `__sloppy.crypto`, bootstrap `sloppy/crypto`, libsodium Argon2id password hash/verify/needsRehash with V8 offload, dependency-backed `NonCryptoHash.xxHash64`, source examples, diagnostic goldens, and a conformance evidence index. | Broader compatibility such as bcrypt/PBKDF2, larger/offloaded streaming hash work, and production hardening remain future work. Random/password/non-crypto tests do not claim randomness quality, cracking-cost evidence, security from non-crypto hashes, timing-proof behavior, WebCrypto/Node/Bun compatibility, or performance. |
| Codec APIs | Green | CORE-CODEC-01.A/B defines the public `sloppy/codec` contract, `stdlib.codec` feature descriptor, compiler Plan activation, backend/dependency policy, stable diagnostics, and non-security checksum boundary. CORE-CODEC-01.C/D/I implements Base64/Base64Url/Hex, UTF-8 encode/decode and streaming decoder, bootstrap exports, generated-app runtime exports, and V8-gated `__sloppy.codec` namespace smoke coverage. CORE-CODEC-01.E implements Binary reader/writer. CORE-CODEC-01.F/G implements zlib-backed gzip/gunzip plus bounded async-iterable gzip/gunzip transforms with decompression-limit diagnostics. CORE-CODEC-01.H/J implements CRC32, checksum security-context warnings, source examples, and conformance evidence indexing. | Broader compression algorithms, incremental native streaming, package evidence, public alpha docs, and production hardening remain future work. Codec tests do not claim Node Buffer, Web Streams, Bun/Deno, checksum security, or performance. |
