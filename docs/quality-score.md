...
| Concurrency/async model | Yellow | Canonical spec and ADR exist, and TASK 09.A adds the first caller-backed single-threaded `SlLoop` completion queue skeleton with CTest coverage. | Add libuv/backend integration, thread-safe posting, owner-thread checks, promise settlement tests, cancellation, backpressure, and worker scaling design. |
