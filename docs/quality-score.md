...
| Network APIs | Yellow | CORE-NET-01.C/D/H adds the Slop-owned native TCP client contract, private libuv connect/read/write/close backend, `TcpClient`/`TcpConnection` stdlib surface, feature-gated `__sloppy.net` V8 bridge, JS-safe TCP resource IDs, owner-thread Promise settlement, and deterministic loopback client conformance including embedded-NUL bytes. CORE-NET-01.E/F adds native `TcpListener`, ephemeral loopback listen, accept, listener resource handles, JS async accept iteration, close/abort, and accept-timeout coverage. CORE-NET-01.G adds `NetworkAddress` object/text parsing, local hostname/DNS-backed listen coverage, IPv4/IPv6 address handling, and `noDelay`/`keepAlive` unsupported-option propagation. | Final deadline/cancellation breadth, stream/backpressure stress, examples, doctor/audit, final conformance/goldens, and live-network evidence remain deferred. |
