...
| Concurrency/async model | Yellow | Canonical spec and ADR exist. TASK 09.A adds the caller-backed single-threaded `SlLoop` completion queue skeleton, and TASK 09.B adds the caller-owned `SlAsync` native settlement skeleton with CTest coverage. | Add libuv/backend integration, thread-safe posting, owner-thread checks, V8 Promise/microtask integration, request scopes, cancellation, backpressure, and worker scaling design. |
