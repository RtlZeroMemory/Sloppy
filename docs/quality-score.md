...
| Security/capabilities | Yellow | Plan/provider/capability metadata, registry checks, provider-executor admission, SQLite bridge enforcement, Plan-driven `sloppy capabilities`/audit visibility for compiler-generated provider effects, CORE-FS-01 filesystem feature/capability policy, core filesystem path-policy enforcement, and resource-table-backed FileHandle IDs. | No OS sandbox; filesystem doctor/audit goldens and watch policy land in later CORE-FS slices; network capabilities remain skeletons; audit remains static metadata review. |
