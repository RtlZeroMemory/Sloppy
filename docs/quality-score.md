...
| Bootstrap stdlib | Yellow | TASK 11.A establishes `stdlib/sloppy/`, CMake staging to `lib/sloppy/bootstrap/sloppy/`, install rules, and a CTest asset-presence check. TASK 11.B/11.C adds frozen `Results.text/json` descriptors, `Sloppy.create`, in-memory `app.mapGet`, `.withName`, `app.__getRoutes`, and a static CTest API-shape check. | Add executable ESM/V8 stdlib tests, compiler import rewriting, `app.plan.json` emission, app host build/freeze/run, HTTP response conversion, and runtime intrinsic binding in later tasks. |
