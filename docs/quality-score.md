...
| Compiler extraction | Yellow | Supported source subset emits deterministic artifacts and diagnostics; source-input run compiles through `sloppyc` and validates generated artifacts through the rebuild-always artifact validation pipeline. COMPILER-30 now defines the source-of-truth inference roadmap, COMPILER-30.A added the module/library/test-harness foundation, and COMPILER-30.B/C adds parser/import/symbol/DSL/static-eval module APIs while preserving artifact compatibility. | Route/group/module extraction beyond current compatibility, provider/config/schema/results metadata, effects/capabilities/completeness, broad TS checking, package/service extraction, cache reuse, broader async source shapes. |
...
library API, diagnostics shape, and fixture/golden harness only; it is not route/provider/
capability inference evidence.
