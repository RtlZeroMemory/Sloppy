...
| Diagnostics | Yellow | Stable codes with completeness coverage, text/JSON/source-frame rendering, JSON source frames when source text is supplied, redaction helpers, provider/runtime diagnostics, compiler source maps with deterministic Sloppy metadata, and V8-gated exception primary-span remapping. | CLI-wide diagnostic format plumbing, structured fixes, richer async stack remapping. |
...
| V8 integration | Yellow | Optional V8 execution, handlers, bounded direct Promise settlement, SQLite bridge, owner-thread checks, async rejection JSON diagnostics, and source-map-remapped exception primary spans. | SDK packaging/CI, ESM module loading, timers/fetch/native async sources, async stack remapping. |
