...
| V8 integration | Yellow | Optional V8 execution, handlers, bounded direct Promise settlement, SQLite bridge, owner-thread checks, execution-domain policy coverage, async completion terminal-guard evidence, async rejection JSON diagnostics, native async scheduler tests, and source-map-remapped exception primary spans. | SQLite bridge not executor-backed, SDK packaging/CI, ESM module loading, timers/fetch/native async sources, async stack remapping. |
...
| Security/capabilities | Yellow | Plan/provider/capability metadata, registry checks, provider-executor admission, SQLite bridge enforcement, Plan-driven `sloppy capabilities`/audit visibility for compiler-generated provider effects, and CORE-FS-01.A/B filesystem feature/capability policy. | No OS sandbox; filesystem operations, filesystem doctor/audit goldens, and runtime enforcement land in later CORE-FS slices; network capabilities remain skeletons; audit remains static metadata review. |
