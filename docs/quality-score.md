...
| Compiler extraction | Yellow | Supported source subset emits deterministic artifacts and diagnostics; source-input run compiles through `sloppyc` and validates generated artifacts through the rebuild-always artifact validation pipeline. COMPILER-30 now defines the source-of-truth inference roadmap, and COMPILER-30.A adds the module/library/test-harness foundation. | Deep Slop DSL inference for routes/groups/modules/providers/config/schema/results/effects/capabilities/completeness, broad TS checking, package/service extraction, cache reuse, broader async source shapes. |
