...
| Memory/string foundations | Yellow | Borrowed `SlStr`/`SlBytes`, arena-owned string/byte copies, deterministic hash/equality helpers, checked math, caller-backed `SlArena`, bounded `SlByteBuilder`/`SlStringBuilder`, bounded app/static `SlInternTable`, arena rollback patterns, diagnostic arena builders, arena-owned Plan/HTTP/SQLite outputs, and JS-safe resource IDs. | Core C tests, builder/intern tests, diagnostics tests, Plan parser tests, HTTP parser/response tests, SQLite provider tests, C standards scanner warnings for raw allocation. | No standalone heap/operation-owned `SlBuf`, no centralized V8/SQLite conversion policy yet (#367), no broad ENGINE-22 hot-path adoption pass, no allocator misuse hard failure, and sanitizer/fuzz hooks remain optional/future. | Complete #367 V8/SQLite interop policy and ENGINE-22 adoption across HTTP, V8, SQLite, diagnostics, Plan/artifacts, CLI, and conformance. |
