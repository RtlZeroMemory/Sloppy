...
| HTTP foundation | Yellow | Bounded parser, route dispatch, response writer, backend lifecycle, libuv localhost transport, sequential keep-alive, chunked behavior, timeout/shutdown diagnostics, and conformance/stress smoke for current lanes. | TLS, production graceful drain, HTTP/2/3, WebSockets, SSE, static files, public streaming APIs, middleware, and production-edge hardening. |
...
| Data providers | Yellow | Native provider foundations, capability checks, provider executor contracts, synchronous SQLite behavior, V8-gated SQLite bridge, and provider metadata/audit surfaces. | Executor-backed SQLite bridge, JavaScript bridges for other providers, live-provider CI lanes, pooling, migrations/schema tooling, and production hardening. |
