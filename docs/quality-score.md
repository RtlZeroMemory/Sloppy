...
| Crypto APIs | Yellow | CORE-CRYPTO-01.E implements OS random helpers, SHA-2/HMAC vectors, ConstantTime, Secret cleanup, `__sloppy.crypto`, bootstrap `sloppy/crypto`, and libsodium Argon2id password hash/verify/needsRehash with V8 offload. | `NonCryptoHash`, final examples, and full conformance/golden pass remain deferred. Random/password tests do not claim randomness quality or cracking-cost evidence. |
