...
| App/framework ergonomics | Yellow | Bootstrap builder/app/config/logging/services/modules/schema/data/result helpers, typed config access, `bind`, config-driven SQLite provider metadata, hardened source-input examples, current compiler/runtime path, ENGINE-16.A/B/C/D/E lifecycle hardening, ENGINE-27.A/B runtime feature registry plus Plan-driven activation, and ENGINE-27.C/D feature descriptors plus Plan-gated V8 intrinsic registration. | Broader generated capability policy, service lifetimes beyond feature validation, formal missing-feature diagnostic goldens, public alpha examples/docs, reload/secrets/custom config providers. |
