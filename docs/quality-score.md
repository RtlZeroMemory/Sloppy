...
| Time/deadline/cancellation | Yellow | CORE-TIME-01.A/B defines the public `sloppy/time` API contract, `stdlib.time` feature descriptor, compiler Plan activation, stable error class names, and representative diagnostic goldens. | Native timers, V8 owner-thread Promise settlement for real timer completions, delay/timeout/cancellation implementation, intervals, scheduled jobs, fake clock, existing-API integration, examples, and conformance remain open. |
