...
| Runtime execution plan | Yellow | Execution model is documented, minimal Plan v1 structs exist, the parser validates a documented golden fixture matrix into arena-owned native data, V8 SDK detection is opt-in, the engine-neutral `SlEngine` ABI has noop/unsupported coverage, and V8-enabled builds have a classic script/global function smoke path. | Add file-based loader, runtime compatibility checks, V8-backed handler-ID execution, and handwritten execution tests later. |
