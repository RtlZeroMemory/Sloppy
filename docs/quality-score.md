...
| Crypto APIs | Yellow | CORE-CRYPTO-01.G implements OS random helpers, SHA-2/HMAC vectors, ConstantTime, Secret cleanup, `__sloppy.crypto`, bootstrap `sloppy/crypto`, libsodium Argon2id password hash/verify/needsRehash with V8 offload, and dependency-backed `NonCryptoHash.xxHash64`. | Final examples and full conformance/golden pass remain deferred. Random/password/non-crypto tests do not claim randomness quality, cracking-cost evidence, security from non-crypto hashes, or performance. |
