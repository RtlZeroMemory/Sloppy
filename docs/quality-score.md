...
| Bootstrap stdlib | Yellow | TASK 11.A establishes `stdlib/sloppy/`, CMake staging to `lib/sloppy/bootstrap/sloppy/`, install rules, and a CTest asset-presence check. The exports are placeholders only. | Implement Results helpers, `Sloppy.create`, `app.mapGet`, compiler rewriting, and runtime intrinsic binding in later tasks. |
