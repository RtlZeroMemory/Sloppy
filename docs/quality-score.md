...
| Crypto APIs | Amber | CORE-CRYPTO-01.C/D/F/H implements OS random helpers, UUID/token/hex/numeric-code shape, SHA-2/HMAC vectors, ConstantTime, Secret cleanup, `__sloppy.crypto`, and the bootstrap `sloppy/crypto` JS surface. | Password hashing/offload, `NonCryptoHash`, final examples, and full conformance/golden pass remain deferred. Random tests do not claim randomness quality. |
