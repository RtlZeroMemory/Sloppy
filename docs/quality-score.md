...
| Crypto APIs | Red | CORE-CRYPTO-01.A/B defines the public `sloppy/crypto` contract, `stdlib.crypto` feature descriptor, compiler Plan activation, backend/password/offload policy, stable diagnostics, and redaction policy. | OS random, SHA-2/HMAC, password hashing, Secret/ConstantTime, NonCryptoHash, V8/stdlib surface, examples, and conformance evidence remain unimplemented. |
