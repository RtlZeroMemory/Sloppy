...
| HTTP/router foundation | Yellow | TASK 10.A adds a pure-C route pattern parser and one-pattern matcher with static segments, string params, int params, parameter captures, diagnostics, and CTest coverage. | Add llhttp/libuv integration, HTTP request parsing, method dispatch, route table/trie or equivalent dispatch, route precedence, percent decoding, route groups, handler dispatch, and public TypeScript API integration later. |
