...
| Security/capabilities | Yellow | Plan/provider/capability metadata, registry checks, provider-executor admission, SQLite bridge enforcement, Plan-driven `sloppy capabilities`/audit visibility for compiler-generated provider effects, CORE-FS-01 filesystem feature/capability policy, and core filesystem path-policy enforcement. | No OS sandbox; filesystem doctor/audit goldens and advanced resource policy land in later CORE-FS slices; network capabilities remain skeletons; audit remains static metadata review. |
