...
The alpha root package references the Windows x64 and Linux x64 GNU platform packages.
macOS npm platform packages are not published in this alpha. Platform packages contain
the native CLI binaries, bootstrap stdlib, built-in project templates, selected
docs/examples, manifest, and license files. The launcher sets `SLOPPY_SLOPPYC` so
