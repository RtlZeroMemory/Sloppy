...
Platform packages contain the native CLI binaries, bootstrap stdlib, built-in
project templates, selected docs/examples, manifest, and license files. The
launcher sets `SLOPPY_SLOPPYC` so commands such as `sloppy create`, `sloppy
build`, and `sloppy package` work outside a repository checkout.
